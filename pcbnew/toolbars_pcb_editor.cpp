...
ACTION_TOOLBAR_CONTROL PCB_ACTION_TOOLBAR_CONTROLS::currentManufacturer( "control.PCBCurrentManufacturer",
                                                                          _( "Preferred Manufacturer" ),
                                                                          _( "Select the preferred PCB manufacturer for this project" ),
                                                                          { FRAME_PCB_EDITOR } );
...
std::optional<TOOLBAR_CONFIGURATION> PCB_EDIT_TOOLBAR_SETTINGS::DefaultToolbarConfig( TOOLBAR_LOC aToolbar )
{
    ...
    case TOOLBAR_LOC::TOP_MAIN:
        ...
        config.AppendAction( PCB_ACTIONS::sendToManufacturer );

        config.AppendSeparator();
        config.AppendAction( PCB_ACTIONS::showEeschema );
        config.AppendControl( PCB_ACTION_TOOLBAR_CONTROLS::currentVariant );
        config.AppendControl( PCB_ACTION_TOOLBAR_CONTROLS::currentManufacturer );
...
void PCB_EDIT_FRAME::configureToolbars()
{
    ...
    // Manufacturer selection dropdown control
    auto manufacturerCtrlFactory =
            [this]( ACTION_TOOLBAR* aToolbar )
            {
                if( !m_currentManufacturerCtrl )
                {
                    wxArrayString choices;
                    choices.Add( _( "(No manufacturer)" ) );
                    choices.Add( wxT( "PCBWay" ) );
                    choices.Add( wxT( "Pikkolo Assembly" ) );

                    m_currentManufacturerCtrl = new wxChoice(
                            aToolbar, ID_TOOLBAR_PCB_SELECT_MANUFACTURER,
                            wxDefaultPosition, wxSize( 160, -1 ), choices );
                }

                m_currentManufacturerCtrl->SetToolTip(
                    _( "Select the preferred manufacturer for DRC and ordering" ) );

                wxString current = Prj().GetProjectFile().m_PreferredManufacturer;
                if( current == wxT( "pcbway" ) )
                    m_currentManufacturerCtrl->SetSelection( 1 );
                else if( current == wxT( "pikkolo" ) )
                    m_currentManufacturerCtrl->SetSelection( 2 );
                else
                    m_currentManufacturerCtrl->SetSelection( 0 );

                aToolbar->Add( m_currentManufacturerCtrl );
            };

    RegisterCustomToolbarControlFactory( PCB_ACTION_TOOLBAR_CONTROLS::currentManufacturer, manufacturerCtrlFactory );
...
void PCB_EDIT_FRAME::ClearToolbarControl( int aId )
{
    ...
    case ID_TOOLBAR_PCB_SELECT_MANUFACTURER: m_currentManufacturerCtrl = nullptr; break;
    }
}
...
void PCB_EDIT_FRAME::onManufacturerSelected( wxCommandEvent& aEvent )
{
    if( !m_currentManufacturerCtrl )
        return;

    int sel = m_currentManufacturerCtrl->GetSelection();
    wxString mfrId;
    ...
    Prj().GetProjectFile().m_PreferredManufacturer = mfrId;
...
void PCB_EDIT_FRAME::ToggleAIChat()
{
    ...
    else
    {
        ...
        appearancePanel.Show( m_ShowLayerManagerTools );
        selectionFilterPanel.Show( m_ShowLayerManagerTools );
...
void PCB_EDIT_FRAME::ShowAIChatPanel()
{
    wxAuiPaneInfo& aiChatPanel = m_auimgr.GetPane( wxS( "AIChat" ) );

    if( aiChatPanel.IsShown() )
        return;
