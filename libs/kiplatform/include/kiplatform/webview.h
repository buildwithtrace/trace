...
namespace WEBVIEW
{
    ...
    /**
     * Execute JavaScript in the given WebView without registering a completion handler.
     *
     * On macOS, wxWebView::RunScriptAsync uses WKWebView's evaluateJavaScript: with a
     * completion handler, whose IPC callback can fire during nested run loop pumping
     * (e.g. progress dialogs) and cause stack overflows in WebKit's JSContext init.
     * This function uses a nil completion handler on macOS to avoid that. On other
     * platforms it falls back to RunScriptAsync.
     *
     * @param aWebView The WebView to execute the script in. Must not be null.
     * @param aScript The JavaScript to execute.
     */
    void RunScriptFireAndForget( wxWebView* aWebView, const wxString& aScript );
