...
void RunScriptFireAndForget( wxWebView* aWebView, const wxString& aScript )
{
    ...
    {
        WKWebView* wkWebView = (__bridge WKWebView*) nativeBackend;
        NSString* nsScript = [NSString stringWithUTF8String:aScript.utf8_str().data()];
        [wkWebView evaluateJavaScript:nsScript completionHandler:nil];
        return;
    }

    aWebView->RunScriptAsync( aScript );
}
