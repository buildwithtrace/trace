...
def calculate_component_bbox(symbol_def, pins, unit, body_style, symbol_pos, symbol_rot, transform_pin_coordinate_func, extract_pin_info_from_symbol_func, mirror=None) -> Optional[Tuple[float, float, float, float]]:
    """
...
def goal_reachable_via_same_net(hit_wire_start, hit_wire_end, goal, same_net_wires, tolerance=0.1):
    """
    Check if the goal position is reachable from the hit wire through
    the network of same-net wires (i.e., they're on the same island).
    
    Uses BFS over wire connectivity.
    """
    goal_tuple = (round(goal[0], 2), round(goal[1], 2))
    
    wire_endpoints = {}
    for ws, we, wd in same_net_wires:
        ws_r = (round(ws[0], 2), round(ws[1], 2))
        we_r = (round(we[0], 2), round(we[1], 2))
        key = id(wd)
        wire_endpoints[key] = (ws_r, we_r)
    
    def points_close(a, b):
        return abs(a[0] - b[0]) < tolerance and abs(a[1] - b[1]) < tolerance
    
    def point_on_wire(pt, ws, we):
        if points_close(pt, ws) or points_close(pt, we):
            return True
        if abs(ws[0] - we[0]) < tolerance:
            if abs(pt[0] - ws[0]) < tolerance:
                min_y = min(ws[1], we[1])
                max_y = max(ws[1], we[1])
                return min_y - tolerance <= pt[1] <= max_y + tolerance
        if abs(ws[1] - we[1]) < tolerance:
            if abs(pt[1] - ws[1]) < tolerance:
                min_x = min(ws[0], we[0])
                max_x = max(ws[0], we[0])
    ...
    hit_s = (round(hit_wire_start[0], 2), round(hit_wire_start[1], 2))
    hit_e = (round(hit_wire_end[0], 2), round(hit_wire_end[1], 2))
    queue.append(hit_s)
    queue.append(hit_e)
    visited.add(hit_s)
    visited.add(hit_e)
    ...
    while queue:
        ...
        if points_close(current, goal_tuple):
            return True
        
        for key, (ws, we) in wire_endpoints.items():
            if points_close(current, ws) and we not in visited:
                visited.add(we)
                queue.append(we)
                if point_on_wire(goal_tuple, ws, we):
                    return True
            elif points_close(current, we) and ws not in visited:
                visited.add(ws)
                queue.append(ws)
                if point_on_wire(goal_tuple, ws, we):
                    return True
...
def astar_pathfind(start: Tuple[float, float],
    ...
    while open_set and iterations < max_iterations:
        ...
        # Emit heartbeat every 5000 iterations to prevent stall timeout
        ...
        # Only terminate early if the goal is reachable via that wire's island
        if current_pos != start_grid and same_net_wires:
            ...
            if hit_wire:
                ...
                if goal_reachable_via_same_net(wire_start, wire_end, goal, same_net_wires, tolerance=grid_size * 0.6):
...
def route_cwires(
    ...
    for comp in components:
        ...
        bbox = calculate_component_bbox(
            ...
            transform_pin_coordinate_func, extract_pin_info_from_symbol_func, bbox_mirror
        )
        if bbox:
