...
class TraceConverter:
    ...
    def convert(self, statements: List[Dict[str, Any]]) -> str:
        ...
        for stmt in statements:
            ...
            if stmt_type in ("file_uid", "paper", "title_block"):
        ...
        for stmt in metadata_statements:
            ...
            elif stmt_type == "title_block":
                self._convert_title_block(stmt)
    ...
    def _format_endpoint(self, endpoint: Dict[str, Any]) -> str:
        """Format a cwire endpoint (pin_ref, junction_ref, or label_ref)."""
        ...
        if endpoint_type == "pin":
            ...
            return endpoint.get("id", "JUNC?")
        elif endpoint_type == "label":
            return endpoint.get("id", "LBL?")
        elif endpoint_type == "glabel":
            return endpoint.get("id", "GLBL?")
        elif endpoint_type == "hier":
            return endpoint.get("id", "HIER?")
        else:
            return "?"
    ...
    def _convert_sheet(self, stmt: Dict[str, Any]):
        ...
        # Optional page (page numbers can be strings like "2A" or integers)
        if "page" in stmt:
            parts.append("page")
            page_val = stmt["page"]
            # Always quote page numbers to handle alphanumeric values like "2A"
            if isinstance(page_val, str):
                parts.append(f'"{self._escape_string(page_val)}"')
            else:
                parts.append(f'"{page_val}"')
    ...
    def _convert_title_block(self, stmt: Dict[str, Any]):
        """Convert a title_block statement."""
        parts = ["title_block"]
        
        if stmt.get("title"):
            parts.append("title")
            parts.append(f'"{self._escape_string(stmt["title"])}"')
        
        if stmt.get("date"):
            parts.append("date")
            parts.append(f'"{self._escape_string(stmt["date"])}"')
        
        if stmt.get("rev"):
            parts.append("rev")
            parts.append(f'"{self._escape_string(stmt["rev"])}"')
        
        if stmt.get("company"):
            parts.append("company")
            parts.append(f'"{self._escape_string(stmt["company"])}"')
        
        comments = stmt.get("comments", {})
        for num in sorted(comments.keys()):
            parts.append("comment")
            parts.append(str(int(num)))
            parts.append(f'"{self._escape_string(comments[num])}"')
