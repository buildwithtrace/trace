...
class TraceConverter:
    ...
    def convert(self, statements: List[Dict[str, Any]]) -> str:
        ...
        for stmt in other_statements:
            ...
            if stmt_type == "footprint":
            ...
            elif stmt_type == "track_arc":
                self._convert_track_arc(stmt)
            elif stmt_type == "via":
                ...
                self._convert_line(stmt)
            elif stmt_type == "arc":
                self._convert_arc(stmt)
            elif stmt_type == "circle":
                self._convert_circle(stmt)
            elif stmt_type == "rect":
                self._convert_rect(stmt)
            elif stmt_type == "curve":
                self._convert_curve(stmt)
            elif stmt_type == "poly":
                self._convert_poly(stmt)
            elif stmt_type == "text":
    ...
    def _convert_edge(self, stmt: Dict[str, Any]):
        """Convert an edge statement (backward-compat)."""
        parts = ["edge"]
    ...
    def _append_optional_layer(self, parts: List[str], stmt: Dict[str, Any]):
        """Append optional layer keyword+value if present."""
        if "layer" in stmt:
            parts.append("layer")
            parts.append(self._format_layer_name(stmt["layer"]))
    
    def _append_optional_stroke(self, parts: List[str], stmt: Dict[str, Any]):
        """Append optional stroke_width and stroke_type if present."""
        if "stroke_width" in stmt:
            parts.append("width")
            parts.append(self._format_number(stmt["stroke_width"]))
        if "stroke_type" in stmt:
            parts.append("stroke")
            parts.append(stmt["stroke_type"])
    
    def _append_optional_fill(self, parts: List[str], stmt: Dict[str, Any]):
        """Append optional fill if present."""
        if "fill" in stmt:
            parts.append("fill")
            parts.append(stmt["fill"])
    
    def _convert_line(self, stmt: Dict[str, Any]):
        """Convert a graphical line statement."""
        parts = ["line"]
        parts.append(self._format_coord(stmt["start"]))
        parts.append("->")
        parts.append(self._format_coord(stmt["end"]))
        self._append_optional_layer(parts, stmt)
        self._append_optional_stroke(parts, stmt)
        parts.append("uid")
        parts.append(stmt["uid"])
        self.lines.append(" ".join(parts))
    
    def _convert_arc(self, stmt: Dict[str, Any]):
        """Convert a graphical arc statement."""
        parts = ["arc"]
        parts.append("start")
        parts.append(self._format_coord(stmt["start"]))
        parts.append("mid")
    ...
    def _convert_circle(self, stmt: Dict[str, Any]):
        """Convert a graphical circle statement."""
        parts = ["circle"]
        parts.append("center")
        ...
        self._append_optional_layer(parts, stmt)
        self._append_optional_stroke(parts, stmt)
        self._append_optional_fill(parts, stmt)
        parts.append("uid")
        parts.append(stmt["uid"])
        self.lines.append(" ".join(parts))
    
    def _convert_rect(self, stmt: Dict[str, Any]):
        """Convert a graphical rectangle statement."""
        parts = ["rect"]
        parts.append("start")
        parts.append(self._format_coord(stmt["start"]))
        parts.append("end")
    ...
    def _convert_curve(self, stmt: Dict[str, Any]):
        """Convert a graphical bezier curve statement."""
        parts = ["curve"]
        parts.append("pts")
        parts.append(self._format_polygon_points(stmt["pts"]))
        self._append_optional_layer(parts, stmt)
        self._append_optional_stroke(parts, stmt)
        parts.append("uid")
        parts.append(stmt["uid"])
    ...
    def _convert_poly(self, stmt: Dict[str, Any]):
        """Convert a graphical polygon statement."""
        parts = ["poly"]
        parts.append("pts")
        parts.append(self._format_polygon_points(stmt["pts"]))
        self._append_optional_layer(parts, stmt)
    ...
    def _convert_track_arc(self, stmt: Dict[str, Any]):
        """Convert a track arc statement."""
        parts = ["track_arc"]
        parts.append("start")
        parts.append(self._format_coord(stmt["start"]))
        parts.append("mid")
        ...
        parts.append(self._format_coord(stmt["end"]))
        parts.append("width")
        parts.append(self._format_number(stmt["width"]))
        parts.append("layer")
        parts.append(self._format_layer_name(str(stmt["layer"])))
        parts.append("net")
        parts.append(str(stmt["net"]))
