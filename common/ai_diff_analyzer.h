...
private:
    ...
    /**
     * Extract the first whitespace-delimited token after a prefix.
     *
     * @param aLine The line to search.
     * @param aPrefix The prefix before the token (e.g., "net ").
     * @return The extracted token, or empty string if not found.
     */
    std::string ExtractFirstToken( const std::string& aLine, const std::string& aPrefix );
