...
void PGM_BASE::Destroy()
{
    ...
    DIAGNOSTIC_CONSOLE::DestroyConsole();
#endif

    TRACE_LOG_FILE::CloseLogFile();
...
bool PGM_BASE::InitPgm( bool aHeadless, bool aSkipPyInit, bool aIsUnitTest )
{
    ...
    if( !aHeadless )
    {
        TRACE_MANAGER::Instance().InitLogging();

#ifdef KICAD_DIAGNOSTIC_LOGGING
        TRACE_MANAGER::Instance().InitDiagnosticConsoleWindow();
#endif
    }
...
wxString GetTraceBackendUrl()
{
    ...
    if( PgmOrNull() )
    {
        ...
        if( settings && !settings->m_System.backend_url.IsEmpty() )
            return settings->m_System.backend_url;
    }
#endif

    wxString envUrl;

    if( wxGetEnv( wxT( "TRACE_BACKEND_URL" ), &envUrl ) && !envUrl.IsEmpty() )
        return envUrl;

#ifdef TRACE_BACKEND_URL
    return wxT( TRACE_BACKEND_URL );
#else
    return wxT( "http://localhost:8000/api/v2" );
#endif
}
