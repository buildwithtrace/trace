...
/**
 * Represents a single todo item tied to a conversation.
 * Used by the AI's TodoWrite/TodoRead tools for task tracking.
 */
struct TODO_ITEM
{
    wxString id;               ///< Unique ID within conversation (e.g. "setup-auth")
    wxString conversation_id;  ///< Foreign key to conversation
    wxString content;          ///< Todo body/description
    wxString status;           ///< "pending", "in_progress", "completed", "cancelled"
    wxString created_at;       ///< ISO timestamp
    wxString updated_at;       ///< ISO timestamp
};
...
public:
    ...
    /**
     * Import a conversation with a specific (server-assigned) ID.
     *
     * Used by the sync path to mirror a Supabase conversation into the local DB using the
     * SAME id the server has, so that subsequent requests carrying that id resolve to the
     * same conversation on both sides. Uses INSERT OR IGNORE: if a row with this id already
     * exists, the call is a no-op.
     *
     * @param aConversationId The explicit UUID to use.
     * @param aUserId User ID.
     * @param aProjectFilePath Project file path (empty is allowed; no "Untitled" collapse).
     * @param aSessionId Session ID.
     * @return The imported conversation, or nullopt on error.
     */
    std::optional<CONVERSATION> ImportConversation( const wxString& aConversationId,
     ...
     * @param aProjectFilePath Optional project file path filter. When non-empty, only
     *        conversations scoped to that project are returned. When empty, conversations
     *        from every project are returned.
     * @return Vector of conversations.
     */
    std::vector<CONVERSATION> ListConversations( const wxString& aUserId = wxEmptyString,
                                                  int aLimit = 50,
                                                  const wxString& aProjectFilePath = wxEmptyString );
    ...
    std::optional<MESSAGE> SaveMessage( const wxString& aConversationId,
                                         ...
                                         const wxString& aMessageId = wxEmptyString );

    /**
     * Update the metadata JSON for an existing message.
     * @param aMessageId The message ID (primary key).
     * @param aMetadata New JSON metadata string.
     * @return True on success.
     */
    bool UpdateMessageMetadata( const wxString& aMessageId, const wxString& aMetadata );

    /**
     * Read the metadata JSON string for a message.
     * @param aMessageId The message ID (primary key).
     * @return The metadata string, or empty string on failure.
     */
    wxString GetMessageMetadata( const wxString& aMessageId );

    /**
     * Delete a single message by its ID.
     * @param aConversationId The conversation ID (for safety).
     * @param aMessageId The message ID to delete.
     * @return True on success.
     */
    bool DeleteMessage( const wxString& aConversationId, const wxString& aMessageId );
    ...
    /**
     * Save or update todos for a conversation.
     * @param aConversationId The conversation ID.
     * @param aTodos Vector of todo items to save.
     * @param aMerge If true, upsert individual items by ID. If false, replace all.
     * @return True if save succeeded.
     */
    bool SaveOrUpdateTodos( const wxString& aConversationId,
                            const std::vector<TODO_ITEM>& aTodos,
                            bool aMerge );

    /**
     * Load all todos for a conversation, ordered by creation time.
     * @param aConversationId The conversation ID.
     * @return Vector of todo items.
     */
    std::vector<TODO_ITEM> LoadTodos( const wxString& aConversationId );
...
private:
    ...
    /**
     * Run one-shot schema/data migrations guarded by `PRAGMA user_version`.
     * Idempotent: each migration bumps user_version so it only runs once per install.
     * @return True on success.
     */
    bool runMigrations();
