...
struct KICOMMON_API CONVERSION_RESULT
{
    ...
    bool        timedOut;     ///< True if process was killed due to stall timeout
    std::string lastPhase;    ///< Last [PHASE] message received (for debugging)
    std::string lintLevel;    ///< Which lint level caught the error: "trace_sch" or "kicad_sch"
    int         exitCode;     ///< Process exit code (1=trace_sch error, 2=kicad_sch validation)
    std::string badContentSnippet; ///< Numbered kicad_sch lines around the error (from [LINT_ERROR])

    CONVERSION_RESULT() : success( false ), timedOut( false ), exitCode( 0 ) {}
    CONVERSION_RESULT( bool aSuccess, const std::string& aError, const std::string& aOutput,
                       bool aTimedOut = false, const std::string& aLastPhase = "",
                       const std::string& aLintLevel = "", int aExitCode = 0 )
        : success( aSuccess ), errorMessage( aError ), output( aOutput ),
          timedOut( aTimedOut ), lastPhase( aLastPhase ),
          lintLevel( aLintLevel ), exitCode( aExitCode )
    {
    }
};
...
struct KICOMMON_API AI_TOOL_RESULT
{
    ...
    // Bad-file reporting fields (populated when conversion validation fails)
    std::string lintLevel;    ///< "trace_sch" or "kicad_sch" if a lint error occurred
    std::string lintError;    ///< Error message from the lint failure
    std::string badContent;   ///< The content that failed validation (for /bad-file report)
    std::string badFilePath;  ///< File path for the bad-file report
...
public:
    ...
    /**
     * Set callback for generating pick-and-place position files from PCB editor.
     * @param aCallback Function that takes JSON options and returns JSON with file path and footprint count.
     */
    void SetPositionFileCallback( std::function<nlohmann::json( const nlohmann::json& )> aCallback )
    {
    ...
    /**
     * Set callback for updating PCB from schematic netlist.
     * This allows direct access to the editor's netlist update functionality.
     * @param aCallback Function that takes JSON options and returns JSON with update results.
     *                  Options: relink_footprints, group_footprints, replace_footprints,
     *                           delete_extra_footprints, override_locks, update_fields,
     *                           remove_extra_fields (all boolean, all optional).
     *                  Output: { "success": bool, "messages": [...], "summary": {...} }
     */
    void SetUpdatePcbFromSchematicCallback( std::function<nlohmann::json( const nlohmann::json& )> aCallback )
    {
        m_updatePcbFromSchematicCallback = aCallback;
    }

    /**
     * Set callback for getting schematic hierarchy information.
     * This allows direct access to the editor's sheet hierarchy.
     * @param aCallback Function that returns JSON with hierarchy info.
     */
    ...
    /**
     * Set callback for switching to a different sheet in the hierarchy.
     * This allows direct access to the editor's sheet navigation.
     * @param aCallback Function that takes JSON params and returns JSON result.
     */
    ...
    /**
     * Set callback for switching to a different layer in PCB editor.
     * @param aCallback Function that takes JSON params and returns JSON result.
     *                  Input: { "layer_name": string, "layer_id": int }
     *                  Output: { "success": bool, "active_layer": string, "layer_id": int }
     */
    ...
    /**
     * Set callback for getting PCB layer information.
     * @param aCallback Function that returns JSON with layer info.
     */
    ...
    /**
     * Set callback for applying a layer preset.
     * @param aCallback Function that takes preset name and returns JSON result.
     */
    ...
    /**
     * Set callback for setting layer visibility.
     * @param aCallback Function that takes layer and visibility params.
     */
    ...
    /**
     * Set callback for fetching footprint dimensions from the PCB editor.
     * @param aCallback Function that takes JSON with footprint list and returns dimensions.
     */
    ...
    /**
     * Set callback for fetching pin information from a KiCad symbol library.
     * @param aCallback Function that takes a symbol library ID string
     *                  (e.g. "Regulator_Switching:XL1509-ADJ") and returns JSON
     *                  with symbol, total_pins, pins[], and units[].
     */
    void SetFetchPinsCallback( std::function<nlohmann::json( const std::string& )> aCallback )
    {
        m_fetchPinsCallback = aCallback;
    }

    /**
     * Set callback for setting the preferred manufacturer on the project file.
     * @param aCallback Function that takes a manufacturer ID string and returns JSON result.
     */
    void SetSetManufacturerCallback( std::function<nlohmann::json( const std::string& )> aCallback )
    {
        m_setManufacturerCallback = aCallback;
    }

    /**
     * Set callback for loading a manufacturer DRC preset.
     * @param aCallback Function that takes a preset ID string and returns JSON result.
     */
    void SetDrcPresetCallback( std::function<nlohmann::json( const std::string& )> aCallback )
    {
        m_drcPresetCallback = aCallback;
    }

    /**
     * Set callback for tracing signal paths in the schematic editor.
     * @param aCallback Function that takes JSON params (net_name, max_hops) and returns
     *                  JSON with paths and connected components.
     */
    void SetTraceSignalPathCallback(
    ...
    /**
     * Set callback for tracing signal paths in the PCB editor.
     * @param aCallback Function that takes JSON params (net_name, max_hops) and returns
     *                  JSON with paths and connected components.
     */
    void SetTracePcbSignalPathCallback(
            std::function<nlohmann::json( const nlohmann::json& )> aCallback )
    {
    ...
    /**
     * Set callback for highlighting a net by name in the editor.
     * @param aCallback Function that takes JSON params (net_name, clear_existing) and
     *                  returns JSON with success/error status.
     */
    void SetHighlightNetCallback(
            std::function<nlohmann::json( const nlohmann::json& )> aCallback )
    {
    ...
    /**
     * Set callback for selecting items by reference designator.
     * @param aCallback Function that takes JSON params (references, zoom_to_fit, clear_existing)
     *                  and returns JSON with selected/not_found lists.
     */
    void SetSelectItemsCallback(
            std::function<nlohmann::json( const nlohmann::json& )> aCallback )
    {
     ...
     */
    void SetConversationId( const std::string& aConversationId )
    {
    ...
    /**
     * Drain all pending before/after trace content snapshots.
     * Returns the snapshots and clears the internal queue.
     * Thread-safe: acquires m_snapshotMutex.
     */
    struct TRACE_CONTENT_SNAPSHOT_DATA
    {
        std::string beforeContent;
        std::string afterContent;
        std::string filePath;
        std::string kicadFilePath;
        bool        conversionSucceeded;

        TRACE_CONTENT_SNAPSHOT_DATA() : conversionSucceeded( false ) {}
    };
    std::vector<TRACE_CONTENT_SNAPSHOT_DATA> DrainPendingSnapshots()
    {
        std::lock_guard<std::mutex> lock( m_snapshotMutex );
        std::vector<TRACE_CONTENT_SNAPSHOT_DATA> result;
        result.reserve( m_pendingSnapshots.size() );
        for( auto& snap : m_pendingSnapshots )
        {
            TRACE_CONTENT_SNAPSHOT_DATA data;
            data.beforeContent       = std::move( snap.beforeContent );
            data.afterContent        = std::move( snap.afterContent );
            data.filePath            = std::move( snap.filePath );
            data.kicadFilePath       = std::move( snap.kicadFilePath );
            data.conversionSucceeded = snap.conversionSucceeded;
    ...
    /**
     * Check if there are pending snapshots to process.
     */
    bool HasPendingSnapshots() const
    {
        std::lock_guard<std::mutex> lock( m_snapshotMutex );
        return !m_pendingSnapshots.empty();
    }
...
private:
    ...
    /**
     * Execute update_pcb_from_schematic tool.
     * Fetches netlist from schematic and updates the PCB board.
     * @param aArgs Tool arguments (all optional boolean flags).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object with update messages and summary.
     */
    AI_TOOL_RESULT executeUpdatePcbFromSchematic( const nlohmann::json& aArgs, const std::string& aKicadFilePath );
    ...
    /**
     * Execute generate_position_file tool.
     * Generates pick-and-place position file from PCB editor.
     * @param aArgs Tool arguments (side, format, smd_only, exclude_dnp, etc.).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object with file path and footprint count.
     */
    AI_TOOL_RESULT executeGeneratePositionFile( const nlohmann::json& aArgs, const std::string& aKicadFilePath );
    ...
    /**
     * Execute get_hierarchy tool.
     * Returns the schematic sheet hierarchy information.
     * @param aArgs Tool arguments (unused).
     * @param aKicadFilePath Path to .kicad_sch file.
     * @return JSON object with hierarchy info.
     */
    AI_TOOL_RESULT executeGetHierarchy( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute switch_sheet tool.
     * Switches to a different sheet in the hierarchy.
     * @param aArgs Tool arguments (page_number, sheet_name, or sheet_path).
     * @param aKicadFilePath Path to .kicad_sch file.
     * @return JSON object with switch result.
     */
    AI_TOOL_RESULT executeSwitchSheet( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute switch_layer tool.
     * Switches to a different layer in the PCB editor.
     * @param aArgs Tool arguments (layer_name or layer_id).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object with switch result.
     */
    AI_TOOL_RESULT executeSwitchLayer( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute get_layers tool.
     * Returns information about all PCB layers.
     * @param aArgs Tool arguments (unused).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object with layer info.
     */
    AI_TOOL_RESULT executeGetLayers( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute apply_layer_preset tool.
     * Applies a layer visibility preset.
     * @param aArgs Tool arguments (preset_name).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object with preset result.
     */
    AI_TOOL_RESULT executeApplyLayerPreset( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute set_layer_visibility tool.
     * Sets visibility of a specific layer.
     * @param aArgs Tool arguments (layer_name or layer_id, visible).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object with visibility result.
     */
    AI_TOOL_RESULT executeSetLayerVisibility( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute fetch_dimensions tool.
     * Returns width/height dimensions for requested footprints on the board.
     * @param aArgs Tool arguments (footprints array).
     * @param aKicadFilePath Path to .kicad_pcb file.
     * @return JSON object mapping footprint identifiers to {width, height} in mm.
     */
    AI_TOOL_RESULT executeFetchDimensions( const nlohmann::json& aArgs, const std::string& aKicadFilePath );

    /**
     * Execute fetch_component_pins tool.
     * Queries the KiCad symbol library for pin information.
     * @param aArgs Tool arguments (symbol: library ID string).
     * @return JSON object with symbol, total_pins, pins[], units[].
     */
    AI_TOOL_RESULT executeFetchComponentPins( const nlohmann::json& aArgs );
     ...
     * Saves or updates todos for the current conversation.
     * @param aArgs Tool arguments (todos array, merge flag).
     * @return JSON result with updated todo list.
     */
    AI_TOOL_RESULT executeTodoWrite( const nlohmann::json& aArgs );

    /**
     * Execute todo_read tool.
     * Loads all todos for the current conversation.
     * @param aArgs Tool arguments (unused).
     * @return JSON array of todo items.
     */
    AI_TOOL_RESULT executeTodoRead( const nlohmann::json& aArgs );

    /**
     * Execute search_symbols tool.
     * Searches for symbols by semantic similarity using local vector index.
     * @param aArgs Tool arguments (query, top_k).
     * @return JSON object with ranked symbol matches.
     */
    AI_TOOL_RESULT executeSearchSymbol( const nlohmann::json& aArgs );

    /**
     * Execute search_footprints tool.
     * Searches for footprints by semantic similarity using local vector index.
     * @param aArgs Tool arguments (query, top_k).
     * @return JSON object with ranked footprint matches.
     */
    AI_TOOL_RESULT executeSearchFootprint( const nlohmann::json& aArgs );

    /**
     * Execute set_preferred_manufacturer tool.
     * Sets the preferred manufacturer in the project file.
     * @param aArgs Tool arguments (manufacturer_id).
     * @return JSON confirmation.
     */
    AI_TOOL_RESULT executeSetPreferredManufacturer( const nlohmann::json& aArgs );

    /**
     * Execute set_manufacturer_drc tool.
     * Loads a manufacturer DRC preset into the current board.
     * @param aArgs Tool arguments (preset_id).
     * @return JSON confirmation with violation count.
     */
    AI_TOOL_RESULT executeSetManufacturerDrc( const nlohmann::json& aArgs );

    /**
     * Execute trace_signal_path tool (schematic).
     * Traces signal paths from a net through the schematic connection graph.
     * @param aArgs Tool arguments (net_name, max_hops).
     * @return JSON with paths and connected components.
     */
    AI_TOOL_RESULT executeTraceSignalPath( const nlohmann::json& aArgs );

    /**
     * Execute trace_pcb_signal_path tool (PCB).
     * Traces signal paths from a net through PCB pad/footprint connectivity.
     * @param aArgs Tool arguments (net_name, max_hops).
     * @return JSON with paths and connected components.
     */
    AI_TOOL_RESULT executeTracePcbSignalPath( const nlohmann::json& aArgs );

    /**
     * Execute highlight_net tool (both editors).
     * Highlights all items belonging to the specified net.
     * @param aArgs Tool arguments (net_name, clear_existing).
     * @return JSON with success/error status.
     */
    AI_TOOL_RESULT executeHighlightNet( const nlohmann::json& aArgs );

    /**
     * Execute select_items tool (both editors).
     * Selects items by reference designator with optional zoom.
     * @param aArgs Tool arguments (references, zoom_to_fit, clear_existing).
     * @return JSON with selected/not_found lists.
     */
    AI_TOOL_RESULT executeSelectItems( const nlohmann::json& aArgs );

    /**
     * Shared implementation for vector search tools.
     * Shells out to the Python local_vector_search.py script.
     * @param aMode "symbol" or "footprint".
     * @param aQueries List of search queries.
     * @param aTopK Number of results to return per query.
     * @param aLibPaths Colon-separated library directory paths.
     * @return JSON search results (dict mapping each query to its results).
     */
    AI_TOOL_RESULT executeVectorSearch( const std::string& aMode,
    ...
    /**
     * Resolve environment variable references in a file path.
     * Handles ${KIPRJMOD} -> aProjectDir and delegates remaining ${VAR}
     * references (e.g. ${TRACE1_TEMPLATE_DIR}) to ExpandEnvVarSubstitutions.
     */
    std::string resolveEnvVars( const std::string& aFilePath,
                                const std::string& aProjectDir );
    ...
    // Gerber/Drill/Position callbacks for direct access to editor generation
    ...
    std::function<nlohmann::json( const nlohmann::json& )> m_positionFileCallback;
    ...
    // Update PCB from schematic callback
    std::function<nlohmann::json( const nlohmann::json& )> m_updatePcbFromSchematicCallback;
    
    // Hierarchy callbacks for schematic sheet navigation
    std::function<nlohmann::json()> m_hierarchyCallback;
    std::function<nlohmann::json( const nlohmann::json& )> m_switchSheetCallback;
    
    // Layer callbacks for PCB layer navigation
    ...
    // Fetch dimensions callback for footprint size queries
    std::function<nlohmann::json( const nlohmann::json& )> m_fetchDimensionsCallback;
    
    // Fetch pins callback for symbol library pin queries (eeschema)
    std::function<nlohmann::json( const std::string& )> m_fetchPinsCallback;

    // Manufacturer callbacks
    std::function<nlohmann::json( const std::string& )> m_setManufacturerCallback;
    std::function<nlohmann::json( const std::string& )> m_drcPresetCallback;

    // Signal path tracer callbacks
    std::function<nlohmann::json( const nlohmann::json& )> m_traceSignalPathCallback;
    std::function<nlohmann::json( const nlohmann::json& )> m_tracePcbSignalPathCallback;

    // Visual feedback callbacks
    std::function<nlohmann::json( const nlohmann::json& )> m_highlightNetCallback;
    std::function<nlohmann::json( const nlohmann::json& )> m_selectItemsCallback;
    ...
    // Before/after trace content for commit-based undo (replaces file-reload approach)
    struct TRACE_CONTENT_SNAPSHOT
    {
        std::string beforeContent;       ///< Trace file content before the edit
        std::string afterContent;        ///< Trace file content after the edit
        std::string filePath;            ///< Path to the trace file
        std::string kicadFilePath;       ///< Path to the converted KiCad file on disk
        bool        conversionSucceeded; ///< Whether syncTraceToKicad succeeded for this edit

        TRACE_CONTENT_SNAPSHOT() : conversionSucceeded( false ) {}
    };
    std::vector<TRACE_CONTENT_SNAPSHOT> m_pendingSnapshots;    ///< Queued snapshots for commit-based undo
    mutable std::mutex                  m_snapshotMutex;       ///< Protects m_pendingSnapshots
    
    // Security: Allowed file extensions for read/write operations
    static const std::set<std::string> s_allowedExtensions;
    // Security: Allowed exact filenames (for extensionless files like fp-lib-table)
    static const std::set<std::string> s_allowedFilenames;
