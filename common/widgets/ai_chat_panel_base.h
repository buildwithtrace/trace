...
struct TAB_DATA
{
    ...
    wxString                       streamStartFilePath;      ///< Schematic file path captured at stream start (immune to mid-stream reload failures)
    ...
    // Symbol preview approval state
    wxString                       pendingSymbolToolCallId;  ///< Non-empty when a symbol preview is awaiting user response

    // Footprint preview approval state
    wxString                       pendingFootprintToolCallId;  ///< Non-empty when a footprint preview is awaiting user response

    // BOM preview approval state
    wxString                       pendingBomToolCallId;  ///< Non-empty when a BOM preview is awaiting user response
...
public:
    ...
    /**
     * Add selected component references to the chat context.
     * Components appear as removable chips in the input area and are prepended to the
     * next message as "Selected components: R1 (10k), C1 (100nF), ...".
     * @param aComponents Vector of (reference, value) pairs, e.g. ("R1", "10k").
     */
    void AddSelectedComponents( const std::vector<std::pair<wxString, wxString>>& aComponents );

    /**
     * Rebind the chat panel to the currently-open project.
     *
     * Called from SCH_EDIT_FRAME::ProjectChanged() / PCB_EDIT_FRAME::ProjectChanged() when the
     * user opens or creates a different project. Persists the current tabs under the OLD project
     * path, cancels any in-flight streams, tears down the in-memory tab state, then reloads the
     * persisted tabs for the NEW project path (creating a fresh empty tab if none exist).
     *
     * Without this, in-memory tabs (conversation IDs, backend clients, streaming state) from the
     * previous project persist across the switch and get re-saved under the new project path,
     * which pulls unrelated prior chats into the new project.
     */
...
protected:
    ...
    /**
     * (LEGACY) Capture the current state before an AI edit.
     * Retained for backward compatibility during transition. Default returns true.
     * @param aFilePath Path to the trace file to backup.
     * @return True if state was captured successfully.
     */
    virtual bool CaptureStateForAIEdit( const wxString& aFilePath ) { return true; }

    /**
     * (LEGACY) Compare the state before and after an AI edit, and create undo entries.
     * Retained for backward compatibility during transition. Default returns true.
     * @return True if comparison and undo entry creation succeeded.
     */
    virtual bool CompareAndCreateAIEditUndoEntries() { return true; }

    /**
     * After ReloadFromFile, remap undo/redo entry pointers to the new schematic
     * items by UUID.  Default is a no-op.
     */
    virtual void RemapUndoRedoAfterReload() {}

    /**
     * Create and configure an AI_COMMIT object for this editor.
     * Derived classes must override to provide editor-specific item resolvers,
     * parsers, and commit objects.
     * @return A configured AI_COMMIT, or nullptr if the editor doesn't support commit-based undo.
     */
    virtual std::unique_ptr<AI_COMMIT> CreateAICommit() { return nullptr; }

    /**
     * Process pending trace content snapshots from the tool executor and apply them
     * through the AI_COMMIT pipeline for native undo/redo support.
     * Called by the batch timer and stream-end handler.
     * @param aTabIndex The tab whose snapshots should be processed.
     */
    void ProcessPendingAIEdits( int aTabIndex );
...
protected:
    ...
    /**
     * Set Position File callback for the AI tool executor.
     * Called by derived PCBnew AI_CHAT_PANEL to enable generate_position_file tool.
     * @param aCallback Callback that generates pick-and-place CSV and returns file path as JSON.
     */
    ...
    /**
     * Set Update PCB from Schematic callback for the AI tool executor.
     * Called by derived PCBnew AI_CHAT_PANEL to enable update_pcb_from_schematic tool.
     * @param aCallback Callback that updates PCB from schematic netlist and returns result as JSON.
     */
    ...
    /**
     * Set callback for fetching symbol pin information from the local KiCad library.
     * @param aCallback Function that takes a symbol library ID string and returns JSON pin data.
     */
     ...
     * Called by derived AI_CHAT_PANEL classes to enable set_preferred_manufacturer tool.
     ...
     * Called by derived AI_CHAT_PANEL classes to enable set_manufacturer_drc tool.
...
protected:
    ...
    bool m_aiEditStateCaptured; ///< True if we've captured state for this edit sequence (LEGACY)

    // Commit-based AI edit pipeline (replaces reload-and-diff approach)
    std::unique_ptr<AI_COMMIT>          m_aiCommit;       ///< Active AI commit session
    std::unique_ptr<AI_EDIT_TRANSLATOR> m_editTranslator; ///< Translates trace diffs to item ops
    ...
    /**
     * Build and show the conversation history menu in the webview.
     * @param aAllProjects When true, include conversations from all projects. When false
     *        (default), only conversations scoped to the current project are shown.
     */
    void showHistoryMenu( bool aAllProjects = false );
    ...
    /**
     * Handle symbol_preview event from backend.
     * Displays the symbol preview and shows approval buttons.
     */
    void handleSymbolPreviewEvent( const AI_BACKEND_EVENT& aEvent, int aTabIndex );
    void handleFootprintPreviewEvent( const AI_BACKEND_EVENT& aEvent, int aTabIndex );

    /**
     * Submit a symbol approval response to the backend via /tools/result.
     */
    void submitSymbolApproval( const std::string& aAction, const std::string& aFeedback,
                               int aTabIndex );
    void submitFootprintApproval( const std::string& aAction, const std::string& aFeedback,
                                  int aTabIndex );

    void handleBomPreviewEvent( const AI_BACKEND_EVENT& aEvent, int aTabIndex );
    void submitBomApproval( const std::string& aAction, const std::string& aFeedback,
                             int aTabIndex );
