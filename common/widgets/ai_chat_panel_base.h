...
struct TAB_DATA
{
    ...
    TAB_DATA() : hasUnsavedChanges( false ), isStreaming( false ), stopRequested( false ), pendingDeltaCount( 0 ), isFirstStreamingFlush( false ), streamingMessageCounter( 0 ), fileModifiedDuringStream( false ), messagesLoaded( false ), isLoadingMessages( false ), idleStatusTimer( nullptr ) {}
...
protected:
     ...
     * 
     * This calls the /api/v1/plan/action endpoint directly, bypassing
    ...
    /**
     * Enable or disable the mode dropdown.
     * Used to make dropdown read-only during plan execution.
     * @param aEnabled True to enable, false to disable.
     */
    void setModeDropdownEnabled( bool aEnabled );
