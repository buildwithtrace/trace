...
/**
 * Return the amount of memory realistically available for new allocations.
 *
 * On Linux, wxGetFreeMemory() returns MemFree which excludes reclaimable
 * page cache and buffers, drastically understating actual availability.
 * This function reads MemAvailable from /proc/meminfo instead, falling
 * back to wxGetFreeMemory() on non-Linux platforms or on parse failure.
 */
static wxMemorySize getAvailableMemory()
{
#ifdef __linux__
        ...
        while( std::getline( meminfo, line ) )
        {
            if( line.compare( 0, 13, "MemAvailable:" ) == 0 )
            {
                ...
                if( sscanf( line.c_str(), "MemAvailable: %lld kB", &kb ) == 1 )
                    return static_cast<wxMemorySize>( kb * 1024LL );
            }
        }
    }
#endif
...
AI_CHAT_PANEL_BASE::AI_CHAT_PANEL_BASE( wxWindow* aParent, EDA_DRAW_FRAME* aFrame ) :
        ...
        m_backendUrl( GetTraceBackendUrl() ),
...
void AI_CHAT_PANEL_BASE::buildUI()
{
    ...
    m_chatWebview->SetHistoryClickCallback( [this]( bool allProjects ) {
        showHistoryMenu( allProjects );
    });
    ...
    m_chatWebview->SetPlanQuestionsAnswerCallback(
        [this]( const wxString& answersJson, const wxString& tabId ) {
            wxLogDebug( wxT( "Plan questions answer callback triggered from webview" ) );
            int tabIndex = resolveTabIndex( tabId );
            if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            {
                m_chatWebview->HidePlanQuestions();
                m_chatWebview->HidePhaseIndicator();
                sendPlanAction( "question_answers", answersJson.ToStdString(), tabIndex );
            }
        });

    // Symbol preview approval callbacks from webview
    m_chatWebview->SetSymbolApproveCallback( [this]( const wxString& toolCallId, const wxString& tabId ) {
        wxLogDebug( wxT( "Symbol approve callback triggered" ) );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitSymbolApproval( "accept", "", tabIndex );
    });

    m_chatWebview->SetSymbolModifyCallback( [this]( const wxString& toolCallId, const wxString& feedback, const wxString& tabId ) {
        wxLogDebug( wxT( "Symbol modify callback triggered: %s" ), feedback );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitSymbolApproval( "modify", feedback.ToStdString(), tabIndex );
    });

    m_chatWebview->SetSymbolCancelCallback( [this]( const wxString& toolCallId, const wxString& tabId ) {
        wxLogDebug( wxT( "Symbol cancel callback triggered" ) );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitSymbolApproval( "cancel", "", tabIndex );
    });

    // Footprint preview callbacks
    m_chatWebview->SetFootprintApproveCallback( [this]( const wxString& toolCallId, const wxString& tabId ) {
        wxLogDebug( wxT( "Footprint approve callback triggered" ) );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitFootprintApproval( "accept", "", tabIndex );
    });

    m_chatWebview->SetFootprintModifyCallback( [this]( const wxString& toolCallId, const wxString& feedback, const wxString& tabId ) {
        wxLogDebug( wxT( "Footprint modify callback triggered: %s" ), feedback );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitFootprintApproval( "modify", feedback.ToStdString(), tabIndex );
    });

    m_chatWebview->SetFootprintCancelCallback( [this]( const wxString& toolCallId, const wxString& tabId ) {
        wxLogDebug( wxT( "Footprint cancel callback triggered" ) );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitFootprintApproval( "cancel", "", tabIndex );
    });

    // BOM preview callbacks
    m_chatWebview->SetBomApproveCallback( [this]( const wxString& toolCallId, const wxString& tabId ) {
        wxLogDebug( wxT( "BOM approve callback triggered" ) );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitBomApproval( "accept", "", tabIndex );
    });

    m_chatWebview->SetBomModifyCallback( [this]( const wxString& toolCallId, const wxString& feedback, const wxString& tabId ) {
        wxLogDebug( wxT( "BOM modify callback triggered: %s" ), feedback );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitBomApproval( "modify", feedback.ToStdString(), tabIndex );
    });

    m_chatWebview->SetBomCancelCallback( [this]( const wxString& toolCallId, const wxString& tabId ) {
        wxLogDebug( wxT( "BOM cancel callback triggered" ) );
        int tabIndex = resolveTabIndex( tabId );
        if( tabIndex >= 0 && tabIndex < static_cast<int>( m_tabs.size() ) )
            submitBomApproval( "cancel", "", tabIndex );
    });
                                                   ...
                                                   const std::vector<ChatAttachment>& aAttachments )
{
    ...
    // If a symbol preview is pending approval, intercept the chat message and
    // route it as a "modify" action with the user's text as feedback.
    if( m_currentTabIndex >= 0 && m_currentTabIndex < static_cast<int>( m_tabs.size() ) )
    {
        TAB_DATA& tab = m_tabs[m_currentTabIndex];
        if( !tab.pendingSymbolToolCallId.IsEmpty() )
        {
            wxLogDebug( wxT( "[SYMBOL] Intercepting chat message as symbol modify feedback" ) );
            submitSymbolApproval( "modify", message.ToStdString(), m_currentTabIndex );
            return;
        }
        if( !tab.pendingFootprintToolCallId.IsEmpty() )
        {
            wxLogDebug( wxT( "[FOOTPRINT] Intercepting chat message as footprint modify feedback" ) );
            submitFootprintApproval( "modify", message.ToStdString(), m_currentTabIndex );
            return;
        }
    }
...
void AI_CHAT_PANEL_BASE::SetPositionFileCallback( std::function<nlohmann::json( const nlohmann::json& )> aCallback )
{
    m_positionFileCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetPositionFileCallback( aCallback );
    }
}
...
void AI_CHAT_PANEL_BASE::SetUpdatePcbFromSchematicCallback( std::function<nlohmann::json( const nlohmann::json& )> aCallback )
{
    m_updatePcbFromSchematicCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetUpdatePcbFromSchematicCallback( aCallback );
    }
}
...
void AI_CHAT_PANEL_BASE::SetFetchPinsCallback( std::function<nlohmann::json( const std::string& )> aCallback )
{
    m_fetchPinsCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetFetchPinsCallback( aCallback );
    }
}

void AI_CHAT_PANEL_BASE::SetTraceSignalPathCallback(
        std::function<nlohmann::json( const nlohmann::json& )> aCallback )
{
    m_traceSignalPathCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetTraceSignalPathCallback( aCallback );
    }
}

void AI_CHAT_PANEL_BASE::SetTracePcbSignalPathCallback(
        std::function<nlohmann::json( const nlohmann::json& )> aCallback )
{
    m_tracePcbSignalPathCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetTracePcbSignalPathCallback( aCallback );
    }
}

void AI_CHAT_PANEL_BASE::SetHighlightNetCallback(
        std::function<nlohmann::json( const nlohmann::json& )> aCallback )
{
    m_highlightNetCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetHighlightNetCallback( aCallback );
    }
}

void AI_CHAT_PANEL_BASE::SetSelectItemsCallback(
        std::function<nlohmann::json( const nlohmann::json& )> aCallback )
{
    m_selectItemsCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetSelectItemsCallback( aCallback );
    }
}
...
void AI_CHAT_PANEL_BASE::SetSetManufacturerCallback( std::function<nlohmann::json( const std::string& )> aCallback )
{
    m_setManufacturerCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetSetManufacturerCallback( aCallback );
    }
}


void AI_CHAT_PANEL_BASE::SetDrcPresetCallback( std::function<nlohmann::json( const std::string& )> aCallback )
{
    m_drcPresetCallback = aCallback;

    for( auto& tab : m_tabs )
    {
        if( tab.toolExecutor )
            tab.toolExecutor->SetDrcPresetCallback( aCallback );
    }
}
...
void AI_CHAT_PANEL_BASE::configureToolExecutor( AI_TOOL_EXECUTOR* aToolExecutor )
{
    ...
    if( m_positionFileCallback )
        aToolExecutor->SetPositionFileCallback( m_positionFileCallback );
    if( m_autorouteCallback )
        aToolExecutor->SetAutorouteCallback( m_autorouteCallback );
    if( m_updatePcbFromSchematicCallback )
        aToolExecutor->SetUpdatePcbFromSchematicCallback( m_updatePcbFromSchematicCallback );
    ...
    if( m_fetchPinsCallback )
        aToolExecutor->SetFetchPinsCallback( m_fetchPinsCallback );
    if( m_traceSignalPathCallback )
        aToolExecutor->SetTraceSignalPathCallback( m_traceSignalPathCallback );
    if( m_tracePcbSignalPathCallback )
        aToolExecutor->SetTracePcbSignalPathCallback( m_tracePcbSignalPathCallback );
    if( m_highlightNetCallback )
        aToolExecutor->SetHighlightNetCallback( m_highlightNetCallback );
    if( m_selectItemsCallback )
        aToolExecutor->SetSelectItemsCallback( m_selectItemsCallback );
    if( m_snapshotCallback )
        aToolExecutor->SetSnapshotCallback( m_snapshotCallback );
    if( m_setManufacturerCallback )
        aToolExecutor->SetSetManufacturerCallback( m_setManufacturerCallback );
    if( m_drcPresetCallback )
        aToolExecutor->SetDrcPresetCallback( m_drcPresetCallback );
...
void AI_CHAT_PANEL_BASE::handleSymbolPreviewEvent( const AI_BACKEND_EVENT& aEvent, int aTabIndex )
{
    if( aTabIndex < 0 || aTabIndex >= static_cast<int>( m_tabs.size() ) )
        return;
    ...
    if( toolCallId.empty() && aEvent.data.contains( "tool_call_id" )
        && aEvent.data["tool_call_id"].is_string() )
    {
        toolCallId = aEvent.data["tool_call_id"].get<std::string>();
    }

    tab.pendingSymbolToolCallId = wxString::FromUTF8( toolCallId );

    std::string symbolJsonStr;
    if( aEvent.data.contains( "symbol_json" ) )
        symbolJsonStr = aEvent.data["symbol_json"].dump();
    else
        symbolJsonStr = "{}";
    ...
    if( !tab.conversationId.IsEmpty() && !tab.conversationId.StartsWith( wxT( "temp_" ) ) )
    {
        CONVERSATION_DB& db = CONVERSATION_DB::Instance();
        nlohmann::json metadata;
        metadata["type"] = "symbol_preview";
        metadata["tool_call_id"] = toolCallId;

        db.SaveMessage(
            tab.conversationId,
            wxT( "assistant" ),
            wxString::FromUTF8( symbolJsonStr ),
            wxString::FromUTF8( metadata.dump() )
        );
    }
    ...
    if( m_chatWebview && aTabIndex == m_currentTabIndex )
    {
        ...
        wxString msgId = wxT( "symbol_preview" );

        m_chatWebview->ShowSymbolPreview(
            msgId,
            wxString::FromUTF8( symbolJsonStr ),
            wxString::FromUTF8( toolCallId )
        );

        m_chatWebview->ShowSymbolApproval();

        tab.streamingMessageId = wxString::Format( wxT( "msg_%lld" ),
            static_cast<long long>( wxGetUTCTimeMillis().GetValue() ) );
        tab.isFirstStreamingFlush = true;
    }
}


void AI_CHAT_PANEL_BASE::submitSymbolApproval( const std::string& aAction,
                                                ...
                                                int aTabIndex )
{
    ...
    TAB_DATA& tab = m_tabs[aTabIndex];
    wxString toolCallId = tab.pendingSymbolToolCallId;

    if( toolCallId.IsEmpty() )
    {
        wxLogWarning( "[SYMBOL] submitSymbolApproval called but no pending tool_call_id" );
        return;
    }

    tab.pendingSymbolToolCallId.Clear();

    if( m_chatWebview )
    {
        m_chatWebview->HideSymbolApproval();
    }
    ...
    std::string tcId = toolCallId.ToStdString();
    std::string sessionId = m_sessionId.ToStdString();
    std::string authToken;
    try
    {
        authToken = AUTH_MANAGER::Instance().GetToken().ToStdString();
    }
    catch( ... ) {}
    ...
    AI_BACKEND_CLIENT* client = m_tabs[aTabIndex].backendClient.get();
    if( !client )
    {
        wxLogWarning( "[SYMBOL] No backend client for tab %d", aTabIndex );
        return;
    }

    std::thread( [client, sessionId, tcId, resultJson, authToken]() {
        client->SubmitToolResult( sessionId, tcId, resultJson, authToken );
    }).detach();
}


void AI_CHAT_PANEL_BASE::handleFootprintPreviewEvent( const AI_BACKEND_EVENT& aEvent,
                                                       int aTabIndex )
{
    if( aTabIndex < 0 || aTabIndex >= static_cast<int>( m_tabs.size() ) )
        return;
    ...
    tab.pendingFootprintToolCallId = wxString::FromUTF8( toolCallId );

    std::string footprintJsonStr;
    if( aEvent.data.contains( "footprint_json" ) )
        footprintJsonStr = aEvent.data["footprint_json"].dump();
    else
        footprintJsonStr = "{}";
    ...
    if( !tab.conversationId.IsEmpty() && !tab.conversationId.StartsWith( wxT( "temp_" ) ) )
    {
        CONVERSATION_DB& db = CONVERSATION_DB::Instance();
        nlohmann::json metadata;
        metadata["type"] = "footprint_preview";
        metadata["tool_call_id"] = toolCallId;

        db.SaveMessage(
            tab.conversationId,
            wxT( "assistant" ),
            wxString::FromUTF8( footprintJsonStr ),
            wxString::FromUTF8( metadata.dump() )
        );
    }
    ...
    if( m_chatWebview && aTabIndex == m_currentTabIndex )
    {
        ...
        wxString msgId = wxT( "footprint_preview" );

        m_chatWebview->ShowFootprintPreview(
            msgId,
            wxString::FromUTF8( footprintJsonStr ),
            wxString::FromUTF8( toolCallId )
        );

        m_chatWebview->ShowFootprintApproval();
...
void AI_CHAT_PANEL_BASE::submitFootprintApproval( const std::string& aAction,
                                                    const std::string& aFeedback,
                                                    int aTabIndex )
{
    if( aTabIndex < 0 || aTabIndex >= static_cast<int>( m_tabs.size() ) )
    ...
    TAB_DATA& tab = m_tabs[aTabIndex];
    wxString toolCallId = tab.pendingFootprintToolCallId;

    if( toolCallId.IsEmpty() )
    {
        wxLogWarning( "[FOOTPRINT] submitFootprintApproval called but no pending tool_call_id" );
        return;
    }

    tab.pendingFootprintToolCallId.Clear();

    if( m_chatWebview )
    {
        m_chatWebview->HideFootprintApproval();
    }
    ...
    if( !client )
    {
        wxLogWarning( "[FOOTPRINT] No backend client for tab %d", aTabIndex );
        return;
    }
...
void AI_CHAT_PANEL_BASE::handleBomPreviewEvent( const AI_BACKEND_EVENT& aEvent,
                                                  int aTabIndex )
{
    if( aTabIndex < 0 || aTabIndex >= static_cast<int>( m_tabs.size() ) )
        return;
    ...
    tab.pendingBomToolCallId = wxString::FromUTF8( toolCallId );

    std::string bomJsonStr = aEvent.data.dump();

    if( !tab.conversationId.IsEmpty() && !tab.conversationId.StartsWith( wxT( "temp_" ) ) )
    {
        CONVERSATION_DB& db = CONVERSATION_DB::Instance();
        nlohmann::json metadata;
        metadata["type"] = "bom_preview";
        metadata["tool_call_id"] = toolCallId;

        db.SaveMessage(
            tab.conversationId,
            wxT( "assistant" ),
            wxString::FromUTF8( bomJsonStr ),
            wxString::FromUTF8( metadata.dump() )
        );
    }
    ...
    if( m_chatWebview && aTabIndex == m_currentTabIndex )
    {
        ...
        wxString msgId = wxT( "bom_preview" );

        m_chatWebview->ShowBomPreview(
            msgId,
            wxString::FromUTF8( bomJsonStr ),
            wxString::FromUTF8( toolCallId )
        );

        m_chatWebview->ShowBomApproval();
...
void AI_CHAT_PANEL_BASE::submitBomApproval( const std::string& aAction,
                                              const std::string& aFeedback,
                                              int aTabIndex )
{
    if( aTabIndex < 0 || aTabIndex >= static_cast<int>( m_tabs.size() ) )
    ...
    TAB_DATA& tab = m_tabs[aTabIndex];
    wxString toolCallId = tab.pendingBomToolCallId;

    if( toolCallId.IsEmpty() )
    {
        wxLogWarning( "[BOM] submitBomApproval called but no pending tool_call_id" );
        return;
    }

    tab.pendingBomToolCallId.Clear();

    if( m_chatWebview )
    {
        m_chatWebview->HideBomApproval();
    }
    ...
    if( !client )
    {
        wxLogWarning( "[BOM] No backend client for tab %d", aTabIndex );
        return;
    }
                                          ...
                                          int aTabIndex )
{
    ...
    // Reset streaming buffer so the first TEXT_DELTA of the new stream
    // creates a fresh assistant message instead of appending to the old one.
...
void AI_CHAT_PANEL_BASE::handleBackendEventDirect( const AI_BACKEND_EVENT& aEvent, int aTabIndex )
{
    ...
    // DEBUG: Log every SSE event with timestamp for diagnosing streaming delays
    {
        ...
        case AI_EVENT_TYPE::SYMBOL_PREVIEW: eventType = wxT( "SYMBOL_PREVIEW" ); break;
        case AI_EVENT_TYPE::BOM_PREVIEW: eventType = wxT( "BOM_PREVIEW" ); break;
    ...
    case AI_EVENT_TYPE::INTERRUPT:
    {
        ...
        if( m_chatWebview )
        {
            ...
            if( interruptType == "questions"
                && aEvent.data.contains( "structured_questions" ) )
            {
                wxString questionsJson = wxString::FromUTF8(
                    aEvent.data["structured_questions"].dump() );
                m_chatWebview->ShowPlanQuestions( questionsJson );
            }
        }
        break;
    }
        ...
        if( aTabIndex >= 0 && aTabIndex < static_cast<int>( m_tabs.size() ) )
        {
            ...
            try
            {
                    ...
                    && aEvent.toolArgs["file_path"].is_string() )
                {
                    ...
                    if( ext == wxT( "kicad_sym" ) )
                        fName = fp;
                    else
                    {
                        fName = fn.GetFullName();
                        if( ext == wxT( "kicad_sch" ) || ext == wxT( "kicad_pcb" ) )
            ...
            if( !fName.IsEmpty() )
            {
                bool isSymLib = fName.EndsWith( wxT( ".kicad_sym" ) );

                if( tName == "read" || tName == "read_file" )
                    marker.label = wxString::Format( wxT( "Read %s" ), fName );
                else if( tName == "write" || tName == "write_file" )
                    marker.label = isSymLib
                        ? wxString::Format( wxT( "Wrote kicad_sym to %s" ), fName )
                        : wxString::Format( wxT( "Wrote %s" ), fName );
    ...
    case AI_EVENT_TYPE::SYMBOL_PREVIEW:
    {
        handleSymbolPreviewEvent( aEvent, aTabIndex );
        break;
    }
    case AI_EVENT_TYPE::FOOTPRINT_PREVIEW:
    {
        handleFootprintPreviewEvent( aEvent, aTabIndex );
        break;
    }
    case AI_EVENT_TYPE::BOM_PREVIEW:
    {
        handleBomPreviewEvent( aEvent, aTabIndex );
        break;
    }
                                               ...
                                               const std::vector<ChatAttachment>& aAttachments )
{
    ...
    if( m_currentTabIndex >= 0 && m_currentTabIndex < static_cast<int>( m_tabs.size() ) )
    {
        ...
        m_tabs[m_currentTabIndex].streamStartFilePath = GetCurrentFileName();
    }
    ...
    if( !currentFile.IsEmpty() && tab.toolExecutor )
    {
        ...
        // Allow read access to the system template directory so the AI agent can
        // read default fp-lib-table / sym-lib-table from ${TRACE1_TEMPLATE_DIR}.
        wxString templateDir = PATHS::GetStockTemplatesPath();
        if( !templateDir.IsEmpty() )
            tab.toolExecutor->AddAllowedProjectDir( templateDir.ToStdString() );
    }

    // Inject preferred manufacturer from the project file
    if( m_frame )
    {
        wxString mfr = m_frame->Prj().GetProjectFile().m_PreferredManufacturer;
        tab.backendClient->SetPreferredManufacturer( mfr.ToStdString() );
    }
                  ...
                  mode, appType, auth, refresh, attachments]()
    {
      try
      {
        ...
        self->CallAfter( [self, panelAlive, convId, result, mode, appType]()
        {
            ...
            if( needsReload )
            {
                // If conversion failed, the .kicad_sch on disk is stale or broken.
                // Don't reload it -- loading a broken file triggers CreateDefaultScreens()
                // which corrupts the filename to "untitled.kicad_sch".
                bool lastConversionOk = !tab->toolExecutor ||
                    tab->toolExecutor->WasLastConversionSuccessful();

                wxString filePath = tab->streamStartFilePath;
                if( filePath.IsEmpty() )
                    filePath = self->GetCurrentFileName();
                wxLogDebug( wxT( "AI DEBUG [stream end]: Processing final AI edits for: %s (conversionOk=%s)" ),
                           filePath, lastConversionOk ? wxT( "true" ) : wxT( "false" ) );
                
                if( filePath.IsEmpty() || !wxFileExists( filePath ) )
                {
                    wxLogWarning( wxT( "AI DEBUG [stream end]: Skipping reload - file path invalid or missing: '%s'" ), filePath );
                }
                else if( !lastConversionOk )
                {
                    wxLogWarning( wxT( "AI DEBUG [stream end]: Skipping reload - last conversion failed, .kicad_sch may be invalid" ) );
                }
                else
                {
                    std::lock_guard<std::mutex> lock( self->m_reloadMutex );

                    // Check conversion success BEFORE reloading to avoid wiping the schematic
                    bool conversionSucceeded = !conversionHappened ||
                        ( tab->toolExecutor && tab->toolExecutor->WasLastConversionSuccessful() );

                    if( !conversionSucceeded )
                    {
                        wxLogWarning( wxT( "AI DEBUG [stream end]: Skipping ReloadFromFile -- conversion failed, keeping current schematic" ) );
                    }
                    else
                    {
                        // Capture the before-state for undo BEFORE any reload
                        ...
                        self->ReloadFromFile( filePath );
                        self->RemapUndoRedoAfterReload();
      ...
      {
          wxLogError( wxT( "AI request thread exception: %s" ), e.what() );
      }
      catch( ... )
      {
          wxLogError( wxT( "AI request thread unknown exception" ) );
      }
    } );
}
...
void AI_CHAT_PANEL_BASE::onStopRequestForTab( int aTabIndex )
{
    ...
    // Revert any in-progress AI commit session (user cancelled mid-stream)
    if( m_aiCommit && m_aiCommit->IsSessionActive() )
    {
        m_aiCommit->Revert();
        m_aiCommit.reset();
        m_editTranslator.reset();
        wxLogDebug( wxT( "AI: Reverted AI commit session due to stop request" ) );
    }
...
{
    // The legacy entry point (wxEvent-based) is still used by a few places;
    // default to project-scoped.
    showHistoryMenu( false );
}


void AI_CHAT_PANEL_BASE::showHistoryMenu( bool aAllProjects )
{
    ...
    // Default to filtering by the current project so users only see chats that belong
    // to the project they're in. Passing an empty path disables the filter.
    wxString projectFilter = aAllProjects ? wxString( wxEmptyString ) : m_cachedProjectPath;

    std::vector<CONVERSATION> conversations = db.ListConversations( userId, 20, projectFilter );
...
void AI_CHAT_PANEL_BASE::reloadMessagesToWebview( const wxString& aConversationId )
{
    ...
    // Deferred symbol preview: same pattern as plan documents.
    wxString pendingSymbolJson;
    wxString pendingSymbolToolCallId;

    // Deferred footprint preview: same pattern as symbol preview.
    wxString pendingFootprintJson;
    wxString pendingFootprintToolCallId;
    ...
    for( const auto& msg : messages )
    {
        ...
        if( msg.role == wxT( "assistant" ) && !msg.metadata.IsEmpty() )
        {
            try
            {
                ...
                if( metadataJson.contains( "type" ) && metadataJson["type"] == "symbol_preview" )
                {
                    pendingSymbolJson = msg.content;
                    pendingSymbolToolCallId = metadataJson.contains( "tool_call_id" )
                        ? wxString::FromUTF8( metadataJson["tool_call_id"].get<std::string>() )
                        : wxString();
                    msgIndex++;
                    continue;
                }

                if( metadataJson.contains( "type" ) && metadataJson["type"] == "footprint_preview" )
                {
                    pendingFootprintJson = msg.content;
                    pendingFootprintToolCallId = metadataJson.contains( "tool_call_id" )
                        ? wxString::FromUTF8( metadataJson["tool_call_id"].get<std::string>() )
                        : wxString();
                    msgIndex++;
                    continue;
                ...
                // Inject preview marker segments so the preview renders
                // at the correct position within the message
                if( !pendingSymbolJson.IsEmpty() || !pendingFootprintJson.IsEmpty() )
                {
                    wxString markers;
                    if( !pendingSymbolJson.IsEmpty() )
                        markers += wxT( "{type:'symbol_preview'}" );
                    if( !pendingFootprintJson.IsEmpty() )
                    {
                        if( !markers.IsEmpty() )
                            markers += wxT( "," );
                        markers += wxT( "{type:'footprint_preview'}" );
                    }

                    if( !segmentsJs.IsEmpty() )
                    {
                        // Append markers before the final ']' of the segments array.
                        // We must use the *last* ']' because inner segments may
                        // contain ']' characters (e.g. labels arrays).
                        size_t lastBracket = segmentsJs.rfind( wxT( ']' ) );
                        if( lastBracket != wxString::npos )
                        {
                            segmentsJs = segmentsJs.Left( lastBracket )
                                       + wxString::Format( wxT( ",%s]" ), markers )
                                       + segmentsJs.Mid( lastBracket + 1 );
                        }
                    }
                    else
                    {
                        if( !textContent.Trim().IsEmpty() )
                        {
                            segmentsJs = wxString::Format(
                                wxT( ",segments:[{type:'text',content:'%s'},%s]" ),
                ...
                // Attach pending symbol preview to this message
                wxString symbolPreviewJs;
                if( !pendingSymbolJson.IsEmpty() )
                {
                    symbolPreviewJs = wxString::Format(
                        wxT( ",symbolPreview:{symbolJson:%s,toolCallId:'%s',"
                             "options:[{id:'accept',label:'Accept'},"
                             "{id:'modify',label:'Modify'},"
                             "{id:'cancel',label:'Cancel'}]}" ),
                        pendingSymbolJson,
                        CHAT_WEBVIEW_PANEL::EscapeForJS( pendingSymbolToolCallId ) );
                }

                // Attach pending footprint preview to this message
                wxString footprintPreviewJs;
                if( !pendingFootprintJson.IsEmpty() )
                {
                    footprintPreviewJs = wxString::Format(
                        wxT( ",footprintPreview:{footprintJson:%s,toolCallId:'%s',"
                             "options:[{id:'accept',label:'Accept'},"
                             "{id:'modify',label:'Modify'},"
                             "{id:'cancel',label:'Cancel'}]}" ),
                        pendingFootprintJson,
                        CHAT_WEBVIEW_PANEL::EscapeForJS( pendingFootprintToolCallId ) );
                }

                if( !segmentsJs.IsEmpty() || !planDocJs.IsEmpty()
                    || !symbolPreviewJs.IsEmpty()
                    || !footprintPreviewJs.IsEmpty() )
                {
                    wxString msgId = wxString::Format( wxT( "reload_%d" ), msgIndex );
                    wxString script = wxString::Format(
                        wxT( "if(window.kichat){window.kichat.addMessage("
                             "{id:'%s',role:'assistant',content:'%s'%s%s%s%s});}" ),
                    ...
                    pendingPlanShowApproval = false;
                    pendingSymbolJson.Clear();
                    pendingSymbolToolCallId.Clear();
                    pendingFootprintJson.Clear();
                    pendingFootprintToolCallId.Clear();
                    msgIndex++;
                    continue;
                }
            }
        ...
        if( ( !pendingSymbolJson.IsEmpty() || !pendingFootprintJson.IsEmpty() )
            && msg.role == wxT( "assistant" ) )
        {
            wxString symJs;
            if( !pendingSymbolJson.IsEmpty() )
            {
                symJs = wxString::Format(
                    wxT( ",symbolPreview:{symbolJson:%s,toolCallId:'%s',"
                         "options:[{id:'accept',label:'Accept'},"
                         "{id:'modify',label:'Modify'},"
                         "{id:'cancel',label:'Cancel'}]}" ),
    ...
    // If a symbol preview was at the very end, emit it standalone
    if( !pendingSymbolJson.IsEmpty() )
    {
        wxString symMsgId = wxT( "symbol_preview" );
        m_chatWebview->ShowSymbolPreview( symMsgId, pendingSymbolJson, pendingSymbolToolCallId );
    }

    // If a footprint preview was at the very end, emit it standalone
    if( !pendingFootprintJson.IsEmpty() )
    {
        wxString fpMsgId = wxT( "footprint_preview" );
        m_chatWebview->ShowFootprintPreview( fpMsgId, pendingFootprintJson, pendingFootprintToolCallId );
    }
}
    ...
    std::thread( [self, panelAlive, convId]()
    {
        ...
        if( panelAlive->load() )
        {
            self->CallAfter( [self, panelAlive, convId, messages]()
            {
                ...
                // If a symbol preview was at the very end, emit it standalone
                if( !pendingSymbolJson.IsEmpty() && self->m_chatWebview
                    && tabIndex == self->m_currentTabIndex )
                {
                    wxString symMsgId = wxT( "symbol_preview" );
                    self->m_chatWebview->ShowSymbolPreview(
                        symMsgId, pendingSymbolJson, pendingSymbolToolCallId );
                }

                // If a footprint preview was at the very end, emit it standalone
                if( !pendingFootprintJson.IsEmpty() && self->m_chatWebview
                    && tabIndex == self->m_currentTabIndex )
                {
                    wxString fpMsgId = wxT( "footprint_preview" );
                    self->m_chatWebview->ShowFootprintPreview(
                        fpMsgId, pendingFootprintJson, pendingFootprintToolCallId );
                }
...
void AI_CHAT_PANEL_BASE::OnProjectChanged()
{
    if( m_isDestroying.load() )
        return;

    wxString oldPath = m_cachedProjectPath;
    wxString newPath = GetCurrentFileName();

    // If the path hasn't changed, there's nothing to rebind. This guards against
    // redundant ProjectChanged notifications (e.g. reopening the same project).
    if( oldPath == newPath )
        return;

    // 1. Persist the current tabs under the OLD project path BEFORE we touch state.
    //    m_cachedProjectPath still reflects the old path, which is what saveOpenTabs uses.
    saveOpenTabs();

    // 2. Cancel any in-flight streams across all tabs. We can't let a pending stream
    //    land on stale tab state (or worse, on a tab that no longer exists).
    for( size_t i = 0; i < m_tabs.size(); i++ )
    {
        TAB_DATA& tab = m_tabs[i];
        ...
        // Detach any joinable request thread; std::thread's destructor would otherwise
        // call std::terminate().
        if( tab.requestThread && tab.requestThread->joinable() )
            tab.requestThread->detach();

        stopIdleStatusTimer( static_cast<int>( i ) );
    }

    // Revert any in-progress AI commit session bound to the old project.
    if( m_aiCommit && m_aiCommit->IsSessionActive() )
    {
        m_aiCommit->Revert();
        m_aiCommit.reset();
    ...
    // 3. Tear down all tabs from the webview and the in-memory vector.
    if( m_chatWebview )
    {
        for( const TAB_DATA& tab : m_tabs )
        {
            if( !tab.conversationId.IsEmpty() )
                m_chatWebview->RemoveTab( tab.conversationId );
        }
    }
    ...
    // Process pending widget deletions before we start adding new ones.
    wxYield();

    // 4. Update the cached path to the NEW project, then reload persisted tabs for it.
    m_cachedProjectPath = newPath;

    if( !loadPersistedTabs() )
    {
        // No persisted tabs for this project - start with a fresh empty tab.
        int newTab = createNewTab();
        if( newTab >= 0 )
            switchToTab( newTab );
    }

    // After rebind, make sure the Send button state reflects reality.
    if( !isAnyTabStreaming() )
        updateButtonState( false );
}
...
void AI_CHAT_PANEL_BASE::saveOpenTabs()
{
    ...
    // Don't persist tabs for unsaved projects: every unsaved project shared a single
    // "Untitled" bucket before this change, which made prior chats re-appear in any
    // new unsaved project. Tabs for unsaved projects live only in memory until the
    // user saves the project and m_cachedProjectPath picks up a real path.
...
bool AI_CHAT_PANEL_BASE::loadPersistedTabs()
{
    try
    {
        ...
        // For unsaved projects (no file path), skip tab restoration. Every unsaved project
        // used to collapse into a single "Untitled" bucket, which pulled unrelated chats
        // back in. Fresh unsaved projects start with an empty tab instead.
        if( projectPath.IsEmpty() )
            return false;
...
void AI_CHAT_PANEL_BASE::HandleFileEditEvent( const AI_BACKEND_EVENT& aEvent, int aTabIndex )
{
    ...
    wxMemorySize availableMemory = getAvailableMemory();
    if( availableMemory != wxNOT_FOUND && availableMemory < 500 * 1024 * 1024 )
    {
        wxLogWarning( wxT( "AI: Low memory (%lld MB available), skipping reload" ),
                     ( availableMemory / (1024*1024) ).GetValue() );
        return;
    }
    ...
    // Flush pending conversions before checking success
    ...
    if( !conversionOk )
    {
        ...
        wxLogWarning( wxT( "AI DEBUG [HandleFileEditEvent]: Skipping ReloadFromFile -- conversion failed: %s" ), convError );
        return;
    }
...
void AI_CHAT_PANEL_BASE::ProcessPendingAIEdits( int aTabIndex )
{
    if( aTabIndex < 0 || aTabIndex >= static_cast<int>( m_tabs.size() ) )
        return;

    TAB_DATA& tab = m_tabs[aTabIndex];
    if( !tab.toolExecutor || !tab.toolExecutor->HasPendingSnapshots() )
        return;

    // Drain all pending before/after snapshots from the tool executor
    auto snapshots = tab.toolExecutor->DrainPendingSnapshots();
    if( snapshots.empty() )
        return;

    // Ensure we have an AI commit session
    if( !m_aiCommit )
    {
        m_aiCommit = CreateAICommit();
        if( !m_aiCommit )
        {
            wxLogWarning( wxT( "AI: Editor does not support commit-based undo" ) );
            return;
        }
    }

    if( !m_aiCommit->IsSessionActive() )
    {
        m_aiCommit->BeginSession( _( "AI Edit" ) );
    }

    // Ensure we have a translator
    if( !m_editTranslator )
    {
        m_editTranslator = std::make_unique<AI_EDIT_TRANSLATOR>(
            GetAppType().ToStdString() );
    }

    // Translate each snapshot and apply ops
    for( const auto& snap : snapshots )
    {
        // Skip snapshots where conversion failed -- the error was already reported
        // to the AI model via the tool result, so it can self-correct.
        if( !snap.conversionSucceeded )
        {
            wxLogDebug( wxT( "AI: Skipping snapshot with failed conversion for %s" ),
                       wxString::FromUTF8( snap.filePath ) );
            continue;
        }

        AI_EDIT_TRANSLATION translation;

        if( !snap.kicadFilePath.empty() )
        {
            // Use the already-converted KiCad file from disk (avoids double-conversion)
            translation = m_editTranslator->TranslateWithKicadFile(
                snap.beforeContent, snap.kicadFilePath, snap.afterContent );
        }
        else
        {
            translation = m_editTranslator->Translate(
                snap.beforeContent, snap.afterContent );
        }

        if( !translation.success )
        {
            wxLogWarning( wxT( "AI: Translation failed: %s" ),
                         wxString::FromUTF8( translation.errorMessage ) );
            continue;
        }

        if( translation.HasChanges() )
        {
            m_aiCommit->ApplyOps( translation.ops );
        }
    }
}
...
void AI_CHAT_PANEL_BASE::onStreamingBatchTimer( wxTimerEvent& aEvent )
{
    ...
    wxString filePath;
    if( m_currentTabIndex >= 0 && m_currentTabIndex < static_cast<int>( m_tabs.size() ) )
        filePath = m_tabs[m_currentTabIndex].streamStartFilePath;
    if( filePath.IsEmpty() )
        filePath = GetCurrentFileName();
    if( filePath.IsEmpty() )
    {
    ...
    if( availableMemory != wxNOT_FOUND && availableMemory < 500 * 1024 * 1024 )
    {
        wxLogWarning( wxT( "AI: Low memory (%lld MB available), skipping batch update" ),
                     ( availableMemory / (1024*1024) ).GetValue() );
        return;
    }

    // Flush pending conversions for ALL streaming tabs
    bool conversionHappened = false;
    bool anyConversionFailed = false;
    for( size_t i = 0; i < m_tabs.size(); i++ )
    {
    ...
    if( !allConversionsOk )
    {
        wxLogWarning( wxT( "AI DEBUG [onStreamingBatchTimer]: Skipping reload -- conversion failed" ) );
        return;
    }

    // During streaming, use ReloadFromFile for real-time visual feedback.
    if( m_aiEditInProgress && !m_aiEditStateCaptured )
    {
        CaptureStateForAIEdit( filePath );
        m_aiEditStateCaptured = true;
...
void AI_CHAT_PANEL_BASE::onReloadDebounceTimer( wxTimerEvent& aEvent )
{
    ...
    // Check if all conversions succeeded BEFORE reloading.
    // If conversion produced invalid kicad_sch, loading it would wipe the
    // schematic (CreateDefaultScreens).  Keep the current good state instead.
    bool allConversionsOk = true;
    for( size_t i = 0; i < m_tabs.size(); i++ )
    {
    ...
    if( !allConversionsOk )
    {
        wxLogWarning( wxT( "AI DEBUG [onReloadDebounceTimer]: SKIPPING ReloadFromFile -- conversion failed, keeping current schematic" ) );
        m_reloadInProgress.store( false );

        if( m_reloadPending.load() )
        {
            m_reloadPending.store( false );
            m_reloadDebounceTimer->StartOnce( 300 );
        }
        return;
    }
...
void AI_CHAT_PANEL_BASE::AddSelectedComponents(
        const std::vector<std::pair<wxString, wxString>>& aComponents )
{
    if( aComponents.empty() || !m_chatWebview )
        return;

    nlohmann::json jsonArray = nlohmann::json::array();

    for( const auto& [ref, value] : aComponents )
    {
        jsonArray.push_back( {
            { "reference", ref.ToStdString() },
            { "value", value.ToStdString() }
        } );
    }

    m_chatWebview->SetSelectedComponents( wxString::FromUTF8( jsonArray.dump() ) );
}
