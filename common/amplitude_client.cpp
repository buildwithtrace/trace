...
void AMPLITUDE_CLIENT::ClearUserId()
{
    std::lock_guard<std::mutex> lock( m_mutex );
    m_userId.clear();
    m_userProperties = nlohmann::json::object();
}


void AMPLITUDE_CLIENT::IdentifyUser()
{
    if( m_apiKey.empty() )
        return;
...

    {
        ...
        if( m_userId.empty() || m_deviceId.empty() )
            return;
