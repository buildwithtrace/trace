...
BEGIN_EVENT_TABLE( KICAD_MANAGER_FRAME, EDA_BASE_FRAME )
    ...
    EVT_MENU( ID_SELECT_PREFERRED_MANUFACTURER, KICAD_MANAGER_FRAME::OnSelectPreferredManufacturer )
...
bool KICAD_MANAGER_FRAME::CloseProject( bool aSave )
{
    ...
    AMPLITUDE_CLIENT::Instance().Track( "project_closed" );
...
bool KICAD_MANAGER_FRAME::LoadProject( const wxFileName& aProjectFileName )
{
    ...
    AMPLITUDE_CLIENT::Instance().Track( "project_opened" );
...
void KICAD_MANAGER_FRAME::CreateNewProject( const wxFileName& aProjectFileName, bool aCreateStubFiles )
{
    ...
    AMPLITUDE_CLIENT::Instance().Track( "project_created" );
}
...
void KICAD_MANAGER_FRAME::OnIdle( wxIdleEvent& aEvent )
{
    ...
    if( !m_crashCheckDone )
    {
        m_crashCheckDone = true;

        std::vector<wxString> crashLogs = TRACE_LOG_FILE::CheckForCrashLogs();
        wxString currentLog = TRACE_LOG_FILE::GetLogFilePath();

        if( !crashLogs.empty() )
        {
            wxString sysInfo = TRACE_LOG_FILE::CollectSystemInfo();

            DIALOG_CRASH_REPORT dlg( this, crashLogs, sysInfo );

            if( dlg.ShowModal() == wxID_OK )
            {
                for( const wxString& logPath : crashLogs )
                    AI_BACKEND_CLIENT::SubmitCrashReport( logPath, sysInfo );
            }
        }

        TRACE_LOG_FILE::CleanupOldLogs( currentLog );
    }
...
void KICAD_MANAGER_FRAME::OnSelectPreferredManufacturer( wxCommandEvent& event )
{
    ...
    wxSingleChoiceDialog dlg( this,
        _( "Select the preferred PCB manufacturer for this project.\n"
           "This affects component selection guidance, DRC presets,\n"
           "and the Send to Manufacturer workflow." ),
        _( "Preferred Manufacturer" ), choices );
...
void KICAD_MANAGER_FRAME::OnAuthPollTimer( wxTimerEvent& event )
{
    ...
    if( currentAuthState != AUTH_STATE::SIGNED_IN )
    {
        // Poll for auth state changes (e.g., user signed in via browser callback).
        // On Windows/Linux the trace:// URL handler launches a new instance that
        // stores the token and exits.  TryRestoreSession() detects the stored
        // token and transitions this (running) instance to SIGNED_IN.
        // The guard inside TryRestoreSession() prevents resetting to SIGNED_OUT
