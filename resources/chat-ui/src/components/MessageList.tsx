...
import FootprintPreviewWidget from './FootprintPreviewWidget'
import BomPreviewWidget from './BomPreviewWidget'
...
const pastTenseMap: Record<string, string> = {
  ...
  'Capturing schematic snapshot...': 'Captured schematic snapshot',
  'One moment...': 'Completed',
  'Still working on it...': 'Completed',
  'Fetching pin information...': 'Fetched pin information',
  'Looking up pin data...': 'Looked up pin data',
  'Reading pin details...': 'Read pin details',
  'Fetching pins for': 'Fetched pin data',
  'Evaluating placement...': 'Evaluated placement',
  'Checking component layout...': 'Checked component layout',
  'Starting component placement...': 'Started placement',
  'Reviewing snapshot': 'Reviewed snapshot',
  'Starting wire routing...': 'Started routing',
  'Routing pass': 'Completed routing pass',
  'Analyzing connections...': 'Analyzed connections',
  'Evaluating routing quality...': 'Evaluated routing quality',
  'Checking wire connections...': 'Checked wire connections',
  'Generating schematic topology...': 'Generated topology',
  'Evaluating review verdict...': 'Completed review',
  'Validating topology and generating skeleton...': 'Generated skeleton',
  'Running schematic review...': 'Ran schematic review',
  'Verifying circuit correctness...': 'Verified circuit correctness',
  'Checking ERC compliance...': 'Checked ERC compliance',
}
...
function InterleavedTextSegment({ content, message, postMessage, tabIsStreaming, showActions }: {
  ...
  const textMsg = useMemo(() => {
    const { planDocument: _pd, symbolPreview: _sp, footprintPreview: _fp, ...rest } = message
    return { ...rest, content }
  }, [message, content])
...
function StoredSegmentsView({ message, postMessage }: {
  ...
  const hasSymbolMarker = segments.some(s => s.type === 'symbol_preview')
  const hasFootprintMarker = segments.some(s => s.type === 'footprint_preview')
  const hasBomMarker = segments.some(s => s.type === 'bom_preview')
  return (
    <>
      {segments.map((seg, i) => {
        if (seg.type === 'symbol_preview') {
          return message.symbolPreview
            ? <SymbolPreviewWidget key={`stored-seg-${i}`} symbolPreview={message.symbolPreview} />
            : null
        }
        if (seg.type === 'footprint_preview') {
          return message.footprintPreview
            ? <FootprintPreviewWidget key={`stored-seg-${i}`} footprintPreview={message.footprintPreview} />
            : null
        }
        if (seg.type === 'bom_preview') {
          return message.bomPreview
            ? <BomPreviewWidget key={`stored-seg-${i}`} bomPreview={message.bomPreview} />
            : null
        }
      ...
      {!hasSymbolMarker && message.symbolPreview && (
        <SymbolPreviewWidget symbolPreview={message.symbolPreview} />
      )}
      {!hasFootprintMarker && message.footprintPreview && (
        <FootprintPreviewWidget footprintPreview={message.footprintPreview} />
      )}
      {!hasBomMarker && message.bomPreview && (
        <BomPreviewWidget bomPreview={message.bomPreview} />
      )}
  ...
  return (
    ...
    >
      ...
      {messages.map((message, idx) => {
        ...
        // Skip standalone preview-only messages when the interleaved turn
        // already renders them via marker segments to avoid duplication.
        if (useInterleaved && message.role === 'assistant' && !message.content) {
          if (message.symbolPreview && turnSegments.some(s => s.type === 'symbol_preview')) return null
          if (message.footprintPreview && turnSegments.some(s => s.type === 'footprint_preview')) return null
          if (message.bomPreview && turnSegments.some(s => s.type === 'bom_preview')) return null
        }

        const isLastUser = idx === lastUserIdx
        const showTodoDrawer = isLastUser && todos.length > 0

        const hasStoredSegments = message.role === 'assistant' && message.segments && message.segments.length > 0
          ...
          <div key={message.id} className={isLastUser ? (showTodoDrawer ? 'todo-message-group' : 'sticky-user-wrapper') : undefined}>
            {hasStoredSegments ? (
              ...
              <>
                ...
                {!hasStoredSegments && message.symbolPreview && (
                  <SymbolPreviewWidget symbolPreview={message.symbolPreview} />
                )}
                {!hasStoredSegments && message.footprintPreview && (
                  <FootprintPreviewWidget footprintPreview={message.footprintPreview} />
                )}
                {!hasStoredSegments && message.bomPreview && (
                  <BomPreviewWidget bomPreview={message.bomPreview} />
                )}
              </>
            )}
      ...
      {useInterleaved && (() => {
        ...
        const hasSymbolMarker = turnSegments.some(s => s.type === 'symbol_preview')
        const hasFootprintMarker = turnSegments.some(s => s.type === 'footprint_preview')
        const hasBomMarker = turnSegments.some(s => s.type === 'bom_preview')
        return (
          <div className="interleaved-turn">
            {turnSegments.map((seg, i) => {
              if (seg.type === 'symbol_preview') {
                const symMsg = messages.find(m => m.symbolPreview)
                return symMsg?.symbolPreview
                  ? <SymbolPreviewWidget key={`seg-${i}`} symbolPreview={symMsg.symbolPreview} />
                  : null
              }
              if (seg.type === 'footprint_preview') {
                const fpMsg = messages.find(m => m.footprintPreview)
                return fpMsg?.footprintPreview
                  ? <FootprintPreviewWidget key={`seg-${i}`} footprintPreview={fpMsg.footprintPreview} />
                  : null
              }
              if (seg.type === 'bom_preview') {
                const bomMsg = messages.find(m => m.bomPreview)
                return bomMsg?.bomPreview
                  ? <BomPreviewWidget key={`seg-${i}`} bomPreview={bomMsg.bomPreview} />
                  : null
              }
              if (seg.type === 'activity') {
            ...
            {!hasSymbolMarker && (() => {
              const symMsg = messages.find(m => m.symbolPreview)
              return symMsg?.symbolPreview
                ? <SymbolPreviewWidget symbolPreview={symMsg.symbolPreview} />
                : null
            })()}
            {!hasFootprintMarker && (() => {
              const fpMsg = messages.find(m => m.footprintPreview)
              return fpMsg?.footprintPreview
                ? <FootprintPreviewWidget footprintPreview={fpMsg.footprintPreview} />
                : null
            })()}
            {!hasBomMarker && (() => {
              const bomMsg = messages.find(m => m.bomPreview)
              return bomMsg?.bomPreview
                ? <BomPreviewWidget bomPreview={bomMsg.bomPreview} />
                : null
            })()}
          </div>
        )
