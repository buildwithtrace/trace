...
import PlanQuestionsBar from './components/PlanQuestionsBar'
import SymbolApprovalBar from './components/SymbolApprovalBar.tsx'
import FootprintApprovalBar from './components/FootprintApprovalBar'
import BomApprovalBar from './components/BomApprovalBar'
import AuthHeader from './components/AuthHeader'
import QuotaBanner from './components/QuotaBanner'
import { useBridge } from './hooks/useBridge'
import type { ChatMessage, ChatMode, TabInfo, ConversationInfo, PhaseIndicatorState, PlanDocument, SymbolPreview, FootprintPreview, BomPreview, FileAttachment, TodoItem, ActivityItem, TurnSegment, ComponentContext, StructuredQuestion } from './vite-env.d'
...
interface TabState {
  ...
  showSymbolApproval: boolean
  pendingSymbolToolCallId: string | null
  showFootprintApproval: boolean
  pendingFootprintToolCallId: string | null
  showBomApproval: boolean
  pendingBomToolCallId: string | null
  showPlanQuestions: boolean
  planQuestions: StructuredQuestion[]
...
const DEFAULT_TAB_STATE: TabState = {
  ...
  showSymbolApproval: false, pendingSymbolToolCallId: null,
  showFootprintApproval: false, pendingFootprintToolCallId: null,
  showBomApproval: false, pendingBomToolCallId: null,
  showPlanQuestions: false, planQuestions: [],
...
function App() {
  ...
  const [selectedComponents, setSelectedComponents] = useState<ComponentContext[]>([])
  ...
  const showSymbolApproval = currentTabState?.showSymbolApproval ?? false
  const showFootprintApproval = currentTabState?.showFootprintApproval ?? false
  const showBomApproval = currentTabState?.showBomApproval ?? false
  const showPlanQuestions = currentTabState?.showPlanQuestions ?? false
  const planQuestions = currentTabState?.planQuestions ?? []
    ...
    updateTabState(tid, state => {
      ...
      if (message.role === 'assistant') {
        if (message.segments && message.segments.length > 0) {
          ...
          const preserved = state.turnSegments.filter(
            seg => seg.type === 'activity' || seg.type === 'symbol_preview' || seg.type === 'footprint_preview' || seg.type === 'bom_preview'
          )
          if (preserved.length > 0) {
            newState.turnSegments = message.content
              ? [...preserved, { type: 'text' as const, content: message.content }]
              : preserved
          }
        }
  ...
  const appendToMessage = useCallback((id: string, text: string, tabId?: string) => {
    ...
    updateTabState(tid, state => {
      ...
      if (lastTextIdx >= 0) {
        segs = [...segs]
        const textSeg = segs[lastTextIdx] as { type: 'text'; content: string }
  ...
  const finalizeMessage = useCallback((id: string, tabId?: string) => {
    ...
    updateTabState(tid, state => {
      ...
      for (const seg of segs) {
        if (seg.type === 'symbol_preview') {
          storedSegments.push({ type: 'symbol_preview' })
        } else if (seg.type === 'footprint_preview') {
          storedSegments.push({ type: 'footprint_preview' })
        } else if (seg.type === 'bom_preview') {
          storedSegments.push({ type: 'bom_preview' })
  ...
  const handleShowPhaseIndicator = useCallback((label: string, color: string, tabId?: string) => {
    ...
    updateTabState(tid, state => {
      return { ...state, phaseIndicator: { label, color } }
    })
  }, [updateTabState, resolveTabId])
  ...
  const handleShowPlanQuestions = useCallback((questionsJson: string, tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    try {
      const questions: StructuredQuestion[] = JSON.parse(questionsJson)
      updateTabState(tid, state => ({ ...state, showPlanQuestions: true, planQuestions: questions }))
    } catch (e) {
      console.error('Failed to parse questions JSON:', e)
    }
  }, [updateTabState, resolveTabId])

  const handleHidePlanQuestions = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showPlanQuestions: false, planQuestions: [] }))
  }, [updateTabState, resolveTabId])

  // --- Symbol preview ---
  const handleShowSymbolPreview = useCallback((messageId: string, symbolJsonStr: string, toolCallId: string) => {
    const tid = resolveTabId(undefined)
    if (!tid) return

    let symbolJson: Record<string, unknown> = {}
    try {
      symbolJson = JSON.parse(symbolJsonStr)
    } catch {
      console.error('[SymbolPreview] Failed to parse symbol JSON')
      return
    }

    const symbolPreview: SymbolPreview = {
      symbolJson,
      toolCallId,
      options: [
    ...
    updateTabState(tid, state => {
      const filtered = state.messages.filter(msg => !msg.symbolPreview || msg.id === messageId)

      const existingMessage = filtered.find(msg => msg.id === messageId)

      const segs: TurnSegment[] = [...state.turnSegments, { type: 'symbol_preview' as const }]

      if (existingMessage) {
        return {
          ...state,
          pendingSymbolToolCallId: toolCallId,
          turnSegments: segs,
          messages: filtered.map(msg =>
            msg.id === messageId ? { ...msg, symbolPreview } : msg
          )
        }
      } else {
        const newMessage: ChatMessage = {
          id: messageId, role: 'assistant', content: '', symbolPreview
        }
        return {
          ...state,
          pendingSymbolToolCallId: toolCallId,
  ...
  const handleShowSymbolApproval = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showSymbolApproval: true }))
  }, [updateTabState, resolveTabId])

  const handleHideSymbolApproval = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showSymbolApproval: false, pendingSymbolToolCallId: null }))
  }, [updateTabState, resolveTabId])

  // --- Footprint preview ---
  const handleShowFootprintPreview = useCallback((messageId: string, footprintJsonStr: string, toolCallId: string) => {
    const tid = resolveTabId(undefined)
    if (!tid) return

    let footprintJson: Record<string, unknown> = {}
    try {
      footprintJson = JSON.parse(footprintJsonStr)
    } catch {
      console.error('[FootprintPreview] Failed to parse footprint JSON')
      return
    }
    ...
    updateTabState(tid, state => {
      const filtered = state.messages.filter(msg => !msg.footprintPreview || msg.id === messageId)

      const existingMessage = filtered.find(msg => msg.id === messageId)

      const segs: TurnSegment[] = [...state.turnSegments, { type: 'footprint_preview' as const }]

      if (existingMessage) {
        return {
          ...state,
          pendingFootprintToolCallId: toolCallId,
          turnSegments: segs,
          messages: filtered.map(msg =>
            msg.id === messageId ? { ...msg, footprintPreview } : msg
          )
        }
      } else {
        const newMessage: ChatMessage = {
          id: messageId, role: 'assistant', content: '', footprintPreview
        }
        return {
          ...state,
          pendingFootprintToolCallId: toolCallId,
  ...
  const handleShowFootprintApproval = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showFootprintApproval: true }))
  }, [updateTabState, resolveTabId])

  const handleHideFootprintApproval = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showFootprintApproval: false, pendingFootprintToolCallId: null }))
  }, [updateTabState, resolveTabId])

  // --- BOM preview ---
  const handleShowBomPreview = useCallback((messageId: string, bomJsonStr: string, toolCallId: string) => {
    const tid = resolveTabId(undefined)
    if (!tid) return

    let bomData: Record<string, unknown> = {}
    try {
      bomData = JSON.parse(bomJsonStr)
    } catch {
      console.error('[BomPreview] Failed to parse BOM JSON')
      return
    }

    const bomPreview: BomPreview = {
      bomItems: (bomData.bom_items as BomPreview['bomItems']) || [],
      projectName: (bomData.project_name as string) || '',
      totalCost: (bomData.total_cost as number | null) ?? null,
      toolCallId,
      options: [
        { id: 'accept', label: 'Accept' },
        { id: 'modify', label: 'Modify' },
    ...
    updateTabState(tid, state => {
      const filtered = state.messages.filter(msg => !msg.bomPreview || msg.id === messageId)
      const existingMessage = filtered.find(msg => msg.id === messageId)
      const segs: TurnSegment[] = [...state.turnSegments, { type: 'bom_preview' as const }]

      if (existingMessage) {
        return {
          ...state,
          pendingBomToolCallId: toolCallId,
          turnSegments: segs,
          messages: filtered.map(msg =>
            msg.id === messageId ? { ...msg, bomPreview } : msg
          )
        }
      } else {
        const newMessage: ChatMessage = {
          id: messageId, role: 'assistant', content: '', bomPreview
        }
        return {
          ...state,
          pendingBomToolCallId: toolCallId,
  ...
  const handleShowBomApproval = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showBomApproval: true }))
  }, [updateTabState, resolveTabId])

  const handleHideBomApproval = useCallback((tabId?: string) => {
    const tid = resolveTabId(tabId)
    if (!tid) return
    updateTabState(tid, state => ({ ...state, showBomApproval: false, pendingBomToolCallId: null }))
  }, [updateTabState, resolveTabId])
  ...
  const { postMessage } = useBridge({
    ...
    showPlanQuestions: handleShowPlanQuestions,
    hidePlanQuestions: handleHidePlanQuestions,
    setPlanDocument: handleSetPlanDocument,
    showSymbolPreview: handleShowSymbolPreview,
    showSymbolApproval: handleShowSymbolApproval,
    hideSymbolApproval: handleHideSymbolApproval,
    showFootprintPreview: handleShowFootprintPreview,
    showFootprintApproval: handleShowFootprintApproval,
    hideFootprintApproval: handleHideFootprintApproval,
    showBomPreview: handleShowBomPreview,
    showBomApproval: handleShowBomApproval,
    hideBomApproval: handleHideBomApproval,
    setTodos: handleSetTodos,
    setSelectedComponents: (components: ComponentContext[]) => {
      setSelectedComponents(prev => {
        const existingRefs = new Set(prev.map(c => c.reference))
        const newComponents = components.filter(c => !existingRefs.has(c.reference))
  ...
  const handleHistoryClick = useCallback((allProjects?: boolean) => {
    postMessage({ type: 'history_click', allProjects: !!allProjects })
  }, [postMessage])
  ...
  const handlePlanQuestionsSubmit = useCallback((answers: Record<string, any>) => {
    const tid = activeTabIdRef.current
    if (!tid) return

    const qs = tabStates.get(tid)?.planQuestions ?? []
    const lines: string[] = []
    for (const q of qs) {
      ...
      if (a.text) {
        ...
        const opt = q.options?.find((o: any) => o.id === a.option)
        const label = opt?.label ?? a.option
        const suffix = a.custom_text ? ` — ${a.custom_text}` : ''
        lines.push(`**${q.id}. ${q.text}**\n${label}${suffix}`)
      }
    }
    ...
    postMessage({ type: 'plan_questions_submit', answers, tabId: tid })
  }, [postMessage, tabStates, updateTabState])

  const handleSymbolApprove = useCallback(() => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingSymbolToolCallId
    if (toolCallId) {
      postMessage({ type: 'symbol_approve', toolCallId, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleSymbolModify = useCallback((feedback: string) => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingSymbolToolCallId
    if (toolCallId) {
      postMessage({ type: 'symbol_modify', toolCallId, feedback, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleSymbolCancel = useCallback(() => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingSymbolToolCallId
    if (toolCallId) {
      postMessage({ type: 'symbol_cancel', toolCallId, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleFootprintApprove = useCallback(() => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingFootprintToolCallId
    if (toolCallId) {
      postMessage({ type: 'footprint_approve', toolCallId, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleFootprintModify = useCallback((feedback: string) => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingFootprintToolCallId
    if (toolCallId) {
      postMessage({ type: 'footprint_modify', toolCallId, feedback, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleFootprintCancel = useCallback(() => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingFootprintToolCallId
    if (toolCallId) {
      postMessage({ type: 'footprint_cancel', toolCallId, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleBomApprove = useCallback(() => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingBomToolCallId
    if (toolCallId) {
      postMessage({ type: 'bom_approve', toolCallId, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleBomModify = useCallback((feedback: string) => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingBomToolCallId
    if (toolCallId) {
      postMessage({ type: 'bom_modify', toolCallId, feedback, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])

  const handleBomCancel = useCallback(() => {
    const tid = activeTabIdRef.current
    const state = tid ? tabStates.get(tid) : undefined
    const toolCallId = state?.pendingBomToolCallId
    if (toolCallId) {
      postMessage({ type: 'bom_cancel', toolCallId, tabId: tid ?? undefined })
    }
  }, [postMessage, tabStates])
  ...
  return (
    <div className={`chat-container ${theme}`}>
      ...
      {showSymbolApproval && (
        <SymbolApprovalBar
          onApprove={handleSymbolApprove}
          onModify={handleSymbolModify}
          onCancel={handleSymbolCancel}
        />
      )}
      {showFootprintApproval && (
        <FootprintApprovalBar
          onApprove={handleFootprintApprove}
          onModify={handleFootprintModify}
          onCancel={handleFootprintCancel}
        />
      )}
      {showBomApproval && (
        <BomApprovalBar
          onApprove={handleBomApprove}
          onModify={handleBomModify}
          onCancel={handleBomCancel}
        />
      )}
      {!isOnline && (
      ...
      <InputArea
        ...
        onRemoveComponent={(ref) => setSelectedComponents(prev => prev.filter(c => c.reference !== ref))}
