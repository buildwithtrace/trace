...
Trace is a KiCad-forked EDA tool (schematic + PCB editor) with an embedded AI chat panel. C++ desktop app (wxWidgets), React/TypeScript webview for the chat UI, Python FastAPI backend for LLM orchestration. The AI reads/writes `.trace_sch` / `.trace_pcb` intermediate files which get converted to KiCad's native `.kicad_sch` / `.kicad_pcb` s-expression format. Current stable version: **1.3.1**.
