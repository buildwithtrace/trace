...
// MSVC does not support _Float16, and MAP_FAILED is POSIX-only.
// Provide software fp16 conversion and define MAP_FAILED for Windows.
...
#elif defined(USEARCH_DEFINED_WINDOWS) && defined(_MSC_VER)
    // Software emulation for MSVC which lacks _Float16 support
    std::uint32_t sign = (u16 >> 15) & 0x1;
    std::uint32_t exp = (u16 >> 10) & 0x1F;
    std::uint32_t mant = u16 & 0x3FF;
...
inline std::uint16_t f32_to_f16(float f32) noexcept {
    ...
    std::uint32_t sign = (f32_bits >> 31) & 0x1;
    std::int32_t exp = ((f32_bits >> 23) & 0xFF) - 127 + 15;
    std::uint32_t mant = (f32_bits >> 13) & 0x3FF;
    std::uint16_t u16;
    if (exp <= 0) {
        u16 = static_cast<std::uint16_t>(sign << 15);
    } else if (exp >= 31) {
        u16 = static_cast<std::uint16_t>((sign << 15) | 0x7C00);
    } else {
        u16 = static_cast<std::uint16_t>((sign << 15) | (exp << 10) | mant);
    }
    return u16;
#else
